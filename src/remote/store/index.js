...
export const useRemoteStore = createWithEqualityFn()(persist(devtools(immer((set, get) => ({
    actions: {
        reconnect: async () => {
            ...
            set((state) => {
                ...
                socket.addEventListener('message', (message) => {
                    ...
                    switch (event) {
                        case 'error': {
                            logFn.error(logMsg[LogCategory.REMOTE].webSocketErrorEvent, {
                                category: LogCategory.REMOTE,
                                meta: { data },
                            });
                        ...
                        case 'favorite': {
                            logFn.debug(logMsg[LogCategory.REMOTE].favoriteEventReceived, {
                                category: LogCategory.REMOTE,
                                meta: {
                        ...
                        case 'playback': {
                            logFn.debug(logMsg[LogCategory.REMOTE].playbackEventReceived, {
                                category: LogCategory.REMOTE,
                                meta: { status: data },
                            });
                            set((state) => {
                        ...
                        case 'position': {
                            logFn.debug(logMsg[LogCategory.REMOTE].positionEventReceived, {
                                category: LogCategory.REMOTE,
                                meta: { position: data },
                            });
                            set((state) => {
                        ...
                        case 'rating': {
                            logFn.debug(logMsg[LogCategory.REMOTE].ratingEventReceived, {
                                category: LogCategory.REMOTE,
                                meta: {
                                    id: data.id,
                                    rating: data.rating,
                        ...
                        case 'repeat': {
                            logFn.debug(logMsg[LogCategory.REMOTE].repeatEventReceived, {
                                category: LogCategory.REMOTE,
                                meta: { repeat: data },
                            });
                            set((state) => {
                        ...
                        case 'shuffle': {
                            logFn.debug(logMsg[LogCategory.REMOTE].shuffleEventReceived, {
                                category: LogCategory.REMOTE,
                                meta: { shuffle: data },
                            });
                            set((state) => {
                ...
                socket.addEventListener('close', (reason) => {
                    ...
                    if (reason.code === 4002 || reason.code === 4003) {
                        ...
                        logFn.error(logMsg[LogCategory.REMOTE].socketClosedUnexpectedly, {
                            category: LogCategory.REMOTE,
                            meta: {
                                code: reason.code,
                                reason: reason.reason,
