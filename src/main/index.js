...
import { app, BrowserWindow, desktopCapturer, globalShortcut, ipcMain, Menu, nativeImage, nativeTheme, net, powerSaveBlocker, protocol, screen, shell, Tray, } from 'electron';
...
import { autoUpdaterLogInterface, createLog, disableAutoUpdates, hotkeyToElectronAccelerator, isLinux, isMacOS, isWindows, } from './utils';
import './features';
import { PlayerRepeat, PlayerStatus, PlayerType } from '/@/shared/types/types';
...
class AppUpdater {
    constructor() {
        const effectiveChannel = store.get('release_channel');
            ...
            checkAllChannelsAndGetBest().then(({ result, updater: updaterInstance }) => {
                ...
                if (isMacOS()) {
                    if (result?.isUpdateAvailable) {
                        getMainWindow()?.webContents.send('update-available', result.updateInfo.version);
                    }
                }
                else {
...
async function checkAllChannelsAndGetBest() {
    ...
    try {
        ...
        if (alphaResult?.updateInfo?.version &&
    ...
    try {
        ...
        if (latestResult?.updateInfo?.version &&
    ...
    const best = candidates.reduce((a, b) => semver.gt(a.result.updateInfo.version, b.result.updateInfo.version) ? a : b);
    if (best.channel === 'latest') {
        configureAutoUpdaterForChannel('latest');
    }
    return { result: best.result, updater: best.updater };
}
function configureAndGetUpdater() {
 ...
 */
function configureAutoUpdaterForChannel(channel) {
    log.transports.file.level = 'info';
    autoUpdater.logger = autoUpdaterLogInterface;
    autoUpdater.autoInstallOnAppQuit = true;
    autoUpdater.autoRunAppAfterInstall = true;
...
if (isLinux() && !process.argv.some((a) => a.startsWith('--password-store='))) {
    const passwordStore = store.get('password_store', 'gnome-libsecret');
...
let currentPlaybackStatus = PlayerStatus.PAUSED;
let currentPrivateMode = false;
let currentRepeatMode = PlayerRepeat.NONE;
let currentSidebarCollapsed = false;
let currentShuffleEnabled = false;
let playbackMenuAccelerators = {};
...
const installExtensions = async () => {
    import('electron-devtools-installer').then((installer) => {
        ...
        installer
            .installExtension(extensions.map((name) => installer[name]), { forceDownload })
...
const getAssetPath = (...paths) => {
    const assetFileName = paths[1];
    if (!app.isPackaged &&
        paths[0] === 'icons' &&
        assetFileName &&
        ['icon.ico', 'icon.png'].includes(assetFileName)) {
        return path.join(__dirname, '../../src/renderer/assets', ...paths);
    }
...
export const sendToastToRenderer = ({ message, type, }) => {
...
async function createWindow(first = true) {
    if (isDevelopment) {
    ...
    if (bounds) {
        ...
        if (bounds.x > screenArea.x + screenArea.width ||
    ...
    const globalMediaKeysEnabled = store.get('global_media_hotkeys', true);
    if (globalMediaKeysEnabled) {
        enableMediaKeys(mainWindow);
    }
    const startWindowMinimized = store.get('window_start_minimized', false);
...
const enableMediaSession = store.get('mediaSession', false);
const playbackType = store.get('playbackType', PlayerType.WEB);
const shouldDisableMediaFeatures = isLinux() || playbackType !== PlayerType.WEB || (!isMacOS() && !enableMediaSession);
const chromiumDisabledFeatures = [];
...
const getMenuAccelerator = (data, action) => {
...
const HOTKEY_ACTIONS = {
    ...
    [BindingActions.PLAY_PAUSE]: () => getMainWindow()?.webContents.send('renderer-player-play-pause'),
    [BindingActions.PREVIOUS]: () => getMainWindow()?.webContents.send('renderer-player-previous'),
    [BindingActions.SHUFFLE]: () => getMainWindow()?.webContents.send('renderer-player-toggle-shuffle'),
    [BindingActions.SKIP_BACKWARD]: () => getMainWindow()?.webContents.send('renderer-player-skip-backward'),
    [BindingActions.SKIP_FORWARD]: () => getMainWindow()?.webContents.send('renderer-player-skip-forward'),
    ...
    [BindingActions.TOGGLE_REPEAT]: () => getMainWindow()?.webContents.send('renderer-player-toggle-repeat'),
    [BindingActions.VOLUME_DOWN]: () => getMainWindow()?.webContents.send('renderer-player-volume-down'),
    [BindingActions.VOLUME_UP]: () => getMainWindow()?.webContents.send('renderer-player-volume-up'),
};
ipcMain.on('set-global-shortcuts', (_event, data) => {
    ...
    for (const shortcut of Object.keys(data)) {
        const isGlobalHotkey = data[shortcut].isGlobal;
        const isValidHotkey = data[shortcut].hotkey &&
            data[shortcut].hotkey !== '';
        if (isGlobalHotkey && isValidHotkey) {
            const accelerator = hotkeyToElectronAccelerator(data[shortcut].hotkey);
            globalShortcut.register(accelerator, () => {
                HOTKEY_ACTIONS[shortcut]();
            });
        }
    }
...
if (!ipcMain.eventNames().includes('open-item')) {
    ipcMain.handle('open-item', async (_event, path) => {
...
ipcMain.on('update-private-mode', (_event, privateMode) => {
    currentPrivateMode = privateMode;
    if (!isMacOS())
        return;
    rebuildMainMenu();
});
ipcMain.on('update-sidebar-collapsed', (_event, collapsedSidebar) => {
