...
function discoverAll(reply) {
    return Promise.all([discoverJellyfin(reply)]);
}
function discoverJellyfin(reply) {
    const sock = createSocket('udp4');
    sock.on('message', (msg) => {
        try {
            const response = JSON.parse(msg.toString('utf-8'));
            reply({
                name: response.Name,
...
ipcMain.on('autodiscover-ping', (ev) => {
    if (ev.ports.length === 0)
        throw new Error('Expected a port to stream autodiscovery results');
