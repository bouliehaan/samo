...
const audiobookshelfProxySessions = new Map();
let audiobookshelfProxyServer = null;
let audiobookshelfProxyPort = null;
const sendAudiobookshelfProxyResponse = (response, statusCode, body, contentType = 'text/plain') => {
...
export const releaseProxySession = (proxySessionId) => {
...
export const releaseProxySessionsForOwner = (ownerSessionId) => {
...
const releaseProxySessionsForWebContents = (webContentsId) => {
    for (const [proxySessionId, session] of audiobookshelfProxySessions) {
...
const registerWebContentsCleanup = (webContentsId) => {
...
const scheduleProxySessionCleanup = (proxySessionId) => {
...
const touchProxySession = (proxySessionId, session) => {
    clearTimeout(session.cleanupTimer);
    ...
    audiobookshelfProxyServer = createServer(async (request, response) => {
        try {
            ...
            if (requestUrl.pathname === '/health') {
                sendAudiobookshelfProxyResponse(response, 200, JSON.stringify({
                    ok: true,
            ...
            const session = audiobookshelfProxySessions.get(proxySessionId);
            if (!session) {
                sendAudiobookshelfProxyResponse(response, 404, 'Unknown Audiobookshelf proxy session');
                return;
            }
            touchProxySession(proxySessionId, session);
            const fetchUpstream = (path) => fetch(`${session.baseUrl}${path}${requestUrl.search}`, {
                headers: {
            ...
            if (!upstreamResponse.ok) {
                sendAudiobookshelfProxyResponse(response, upstreamResponse.status, `Audiobookshelf proxy request failed: ${upstreamResponse.status}`);
                return;
            }
...
export const createAudiobookshelfProxyUrl = async (baseUrl, token, contentUrl, ownerSessionId, webContentsId) => {
    const port = await ensureAudiobookshelfProxyServer();
    evictOldestProxySessionIfNeeded();
    const proxySessionId = randomUUID();
    const normalizedBaseUrl = normalizeBaseUrl(baseUrl);
    const url = contentUrl.startsWith('http://') || contentUrl.startsWith('https://')
        ? new URL(contentUrl)
