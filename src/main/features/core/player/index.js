...
let mpvLifecyclePromise = Promise.resolve();
...
const mpvLog = (data, err) => {
    const { action, toast, type = 'info' } = data;
    if (err) {
        const errcode = 'errcode' in err ? err.errcode : undefined;
        const codeDescription = typeof errcode === 'number'
            ? `mpv errorcode ${errcode} - ${NodeMpvErrorCode[errcode] ??
                'Unknown MPV error'}`
            : undefined;
...
const getConfiguredMpvBinaryPath = () => store.get('mpv_path');
...
const getPackagedMacOSMpvCandidates = () => {
...
const isExecutableName = (value) => !/[\\/]/.test(value);
const ensureMpvCandidateExecutable = ({ canRepairPermissions, path: candidatePath, source, }) => {
    const stats = statSync(candidatePath);
    if (!stats.isFile()) {
        throw new Error(`MPV unavailable: ${source} exists at ${candidatePath}, but it is not a file.`);
    }
    try {
        ...
        throw new Error(`MPV unavailable: ${source} exists at ${candidatePath}, but it is not executable.`);
    }
};
const resolveExistingMpvCandidate = (candidate) => {
...
const logSelectedMpvPath = (mode, path, source) => {
    mpvLog({
        action: `Resolved MPV binary in ${mode} mode from ${source}: ${path}`,
        type: 'info',
    });
};
const resolveMpvBinaryPath = (binaryPath) => {
    ...
    if (isMacOS() && app.isPackaged) {
        ...
        throw new Error(`MPV unavailable: packaged macOS builds require bundled MPV at ${getBundledMacOSMpvBinaryPath()} with optional Intel backup at ${getBundledMacOSX64MpvBinaryPath()}. Populate resources/bin/darwin before building release artifacts.`);
    }
    if (isMacOS()) {
        const candidates = [
        ...
        throw new Error(`MPV unavailable: set SAMO_MPV_PATH, configure mpv_path, or install mpv at ${MACOS_DEV_MPV_CANDIDATES.join(' or ')}.`);
    }
    const candidates = [
...
const DEFAULT_MPV_PARAMETERS = (extraParameters) => {
...
const attachMpvProcessLogging = (instance) => {
    ...
    mpvProcess.once('exit', (code, signal) => {
        ...
        mpvLog({
            action: childPid !== undefined
                ? `Exited mpv child process pid=${childPid} code=${code ?? 'null'} signal=${signal ?? 'null'}`
                : `Exited mpv child process pid unavailable code=${code ?? 'null'} signal=${signal ?? 'null'}`,
            type: 'info',
        });
    });
};
const hasMpvChildProcessExited = (mpvProcess) => {
    return mpvProcess?.exitCode != null || mpvProcess?.signalCode != null;
};
const terminateMpvProcess = async (instance, reason, options) => {
    if (isWindows()) {
        return;
    }
    const mpvProcess = getMpvChildProcess(instance);
    ...
    try {
        mpvLog({
            action: typeof mpvProcess.pid === 'number'
        ...
        mpvLog({ action: 'Failed to terminate native mpv process' }, killErr);
        return;
    }
    await wait(MPV_QUIT_GRACE_PERIOD_MS);
    if (hasMpvChildProcessExited(mpvProcess)) {
    ...
    try {
        ...
        mpvLog({ action: 'Failed to force kill native mpv process' }, killErr);
    }
};
const quit = async (instance) => {
    ...
    if (mpv) {
        ...
        try {
            ...
            await Promise.race([
                mpv.quit(),
                new Promise((_, reject) => setTimeout(() => reject(new Error(`mpv quit IPC timed out after ${MPV_QUIT_IPC_TIMEOUT_MS}ms`)), MPV_QUIT_IPC_TIMEOUT_MS)),
            ]);
        }
        catch (error) {
            mpvLog({
                action: `mpv quit IPC failed; terminating native process directly: ${error?.message ?? String(error)}`,
                type: 'warning',
            });
        }
...
const runMpvLifecycle = async (task) => {
    const run = mpvLifecyclePromise.then(task, task);
    mpvLifecyclePromise = run.then(() => undefined, () => undefined);
    return run;
};
const shutdownMpvInstance = async (instance, reason, options) => {
    if (!instance) {
        return;
    }
...
ipcMain.on('player-set-properties', async (_event, data) => {
...
ipcMain.handle('player-restart', async (_event, data) => {
    try {
...
ipcMain.handle('player-initialize', async (_event, data) => {
    try {
        await runMpvLifecycle(async () => {
            mpvLog({
...
ipcMain.on('player-seek', async (_event, time) => {
    try {
...
ipcMain.on('player-seek-to', async (_event, time) => {
    try {
...
ipcMain.on('player-set-queue', async (_event, current, next, pause) => {
    if (!current && !next) {
        try {
...
ipcMain.on('player-set-queue-next', async (_event, url) => {
    try {
...
ipcMain.on('player-auto-next', async (_event, url) => {
...
ipcMain.on('player-volume', async (_event, value) => {
    try {
...
ipcMain.on('player-mute', async (_event, mute) => {
    try {
...
ipcMain.on('player-update-metadata', (_event, data) => {
    currentPlayerData = data;
});
// Returns the current player metadata (song data)
ipcMain.handle('player-metadata', async () => {
    return currentPlayerData;
});
// Returns the stream metadata from mpv (for radio streams)
const parseIcyStreamTitle = (raw) => {
...
const fetchIcyMetadata = (streamUrl, redirectCount = 0) => {
    return new Promise((resolve) => {
        let settled = false;
        const finish = (value) => {
            if (settled)
        ...
        try {
            ...
            const request = client.request(url, {
                ...
                if (!Number.isFinite(metaint) || metaint <= 0) {
                    response.destroy();
                    finish(typeof icyName === 'string' && icyName.trim()
...
ipcMain.handle('player-stream-metadata', async (_event, streamUrl) => {
    try {
        if (streamUrl) {
    ...
    try {
        ...
        if (instance && instance.isRunning()) {
            ...
            try {
                ...
                mpvLog({ action: 'Failed to create temporary MPV instance for audio device list' }, err);
                return [];
            }
        }
        try {
...
var MpvState;
(function (MpvState) {
    MpvState[MpvState["STARTED"] = 0] = "STARTED";
    MpvState[MpvState["IN_PROGRESS"] = 1] = "IN_PROGRESS";
    MpvState[MpvState["DONE"] = 2] = "DONE";
})(MpvState || (MpvState = {}));
