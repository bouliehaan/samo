...
export const enableMediaKeys = (window) => {
    if (isMacOS()) {
        const shouldPrompt = store.get('should_prompt_accessibility', true);
        const shownWarning = store.get('shown_accessibility_warning', false);
        ...
        if (!trusted && !shownWarning) {
            window?.webContents.send('toast-from-main', {
                message: 'Samo is not a trusted accessibility client. Media keys will not work until this setting is changed',
                type: 'warning',
            });
    ...
    const useGlobalShortcut = isLinux() || playbackType !== PlayerType.WEB || (!isMacOS() && !enableMediaSession);
