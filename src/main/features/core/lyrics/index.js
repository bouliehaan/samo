...
import { getLyricsBySongId as getLrcLib, query as queryLrcLib, getSearchResults as searchLrcLib, } from './lrclib';
...
const setCache = (key, result) => {
    ...
    if (cache.size > CACHE_MAX_ENTRIES) {
        const oldestKey = [...cache.entries()].sort((a, b) => a[1].updatedAt - b[1].updatedAt)[0]?.[0];
        if (oldestKey)
            cache.delete(oldestKey);
    }
    scheduleCacheWrite();
};
const buildSearchParams = (song) => ({
...
const fetchFromLrcLib = async (song, signal) => {
...
const getRemoteLyrics = async (song) => {
    await loadCache();
...
const searchRemoteLyrics = async (params) => {
    const empty = {
        [LyricSource.GENIUS]: [],
        [LyricSource.LRCLIB]: [],
...
const getRemoteLyricsById = async (params) => {
    if (isOfflineMode())
        return null;
    if (params.remoteSource !== LyricSource.LRCLIB)
        return null;
    return getLrcLib(params.remoteSongId).catch(() => null);
};
ipcMain.handle('lyric-by-song', async (_event, song) => getRemoteLyrics(song));
ipcMain.handle('lyric-search', async (_event, params) => searchRemoteLyrics(params));
ipcMain.handle('lyric-by-remote-id', async (_event, params) => getRemoteLyricsById(params));
ipcMain.handle('lyric-clear-cache-for-song', async (_event, song) => {
    await loadCache();
    const key = cacheKey(song);
    cache.delete(key);
    scheduleCacheWrite();
