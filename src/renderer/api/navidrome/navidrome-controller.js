...
import { getFeatures, hasFeature, hasFeatureWithVersion } from '/@/shared/api/utils';
import { albumArtistListSortMap, albumListSortMap, genreListSortMap, playlistListSortMap, SongListSort, songListSortMap, SortOrder, sortOrderMap, tagListSortMap, userListSortMap, } from '/@/shared/types/domain-types';
...
const EXCLUDED_ALBUM_TAGS = new Set([
    'asin',
    'barcode',
    'copyright',
    'encodedby',
...
const EXCLUDED_SONG_TAGS = new Set(['disctotal', 'tracktotal']);
...
const ID_TAGS = new Set(['albumversion', 'mood']);
const excludeMissing = (server) => {
    if (!server) {
        return undefined;
    }
...
const getLibraryId = (musicFolderId) => {
    if (!musicFolderId) {
        return undefined;
    }
    return Array.isArray(musicFolderId) ? musicFolderId : [musicFolderId];
};
const getArtistSongKey = (server) => hasFeature(server, ServerFeature.TRACK_ALBUM_ARTIST_SEARCH) ? 'artists_id' : 'album_artist_id';
export const NavidromeController = {
    ...
    deleteArtistImage: async (args) => {
        const { apiClientProps, query } = args;
        const res = await ndApiClient(apiClientProps).deleteArtistImage({
            params: {
                id: query.id,
            },
        });
    ...
    deleteInternetRadioStationImage: async (args) => {
        const { apiClientProps, query } = args;
        const res = await ndApiClient(apiClientProps).deleteInternetRadioStationImage({
            params: {
                id: query.id,
            },
        });
    ...
    deletePlaylistImage: async (args) => {
        const { apiClientProps, query } = args;
        const res = await ndApiClient(apiClientProps).deletePlaylistImage({
            params: {
                id: query.id,
            },
        });
    ...
    getAlbumArtistInfo: async (args) => {
        ...
        const imageUrl = artistInfo?.largeImageUrl ||
    ...
    getAlbumArtistListCount: async ({ apiClientProps, query }) => NavidromeController.getAlbumArtistList({
        apiClientProps,
    ...
    getAlbumDetail: async (args) => {
        ...
        return ndNormalize.album({ ...albumRes.body.data, songs: songsData.body.data }, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith);
    },
    ...
    getAlbumList: async (args) => {
        ...
        return {
            items: res.body.data.map((album) => ndNormalize.album(album, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith)),
            startIndex: query?.startIndex || 0,
            totalRecordCount: Number(res.body.headers.get('x-total-count') || 0),
        };
    },
    getAlbumListCount: async ({ apiClientProps, query }) => NavidromeController.getAlbumList({
        apiClientProps,
        query: { ...query, limit: 1, startIndex: 0 },
    }).then((result) => result.totalRecordCount),
    getAlbumRadio: async (args) => {
        ...
        return res.body.similarSongs.song.map((song) => ssNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
    },
    ...
    getArtistListCount: async ({ apiClientProps, query }) => NavidromeController.getArtistList({
        apiClientProps,
        query: { ...query, limit: 1, startIndex: 0 },
    }).then((result) => result.totalRecordCount),
    getArtistRadio: async (args) => {
        ...
        return res.body.similarSongs2.song.map((song) => ssNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
    },
    ...
    getGenreList: async (args) => {
        ...
        if (hasFeature(apiClientProps.server, ServerFeature.BFR)) {
            ...
            return {
                items: res.body.data.map((genre) => ndNormalize.genre({
    ...
    getPlaylistListCount: async ({ apiClientProps, query }) => NavidromeController.getPlaylistList({
        apiClientProps,
        query: { ...query, limit: 1, startIndex: 0 },
    }).then((result) => result.totalRecordCount),
    getPlaylistSongList: async (args) => {
        ...
        return {
            items: res.body.data.map((item) => ndNormalize.song(item, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)),
            startIndex: 0,
            totalRecordCount: Number(res.body.headers.get('x-total-count') || 0),
        };
    },
    getPlayQueue: async (args) => {
        ...
        if (hasFeatureWithVersion(apiClientProps.server, ServerFeature.SERVER_PLAY_QUEUE, 2)) {
            ...
            const entries = items.map((song) => ndNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
            return {
                changed: updatedAt,
                changedBy,
    ...
    getRoles: async ({ apiClientProps }) => hasFeature(apiClientProps.server, ServerFeature.BFR) ? NAVIDROME_ROLES : [],
    getServerInfo: async (args) => {
        ...
        if (subsonicArgs.features.serverPlayQueue && navidromeFeatures.serverPlayQueue) {
            features.serverPlayQueue = navidromeFeatures.serverPlayQueue.concat(subsonicArgs.features.serverPlayQueue);
        }
        return {
            features,
    ...
    getSimilarSongs: async (args) => {
        ...
        return ((res.body.similarSongs?.song || [])
            .filter((song) => song.id !== query.songId)
            .map((song) => ssNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)) || []);
    },
    getSongDetail: async (args) => {
        ...
        return ndNormalize.song(res.body.data, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith);
    },
    getSongList: async (args) => {
        ...
        const fetchAlbums = async (albumIdBatch) => {
            const res = await ndApiClient(apiClientProps).getSongList({
                query: {
                    ...
                    ...(hasFeature(apiClientProps.server, ServerFeature.TRACK_YES_NO_RATING_FILTER) && query.hasRating !== undefined
                        ? { has_rating: query.hasRating }
                        : {}),
                    library_id: getLibraryId(query.musicFolderId),
                    starred: query.favorite,
            ...
            return {
                items: res.body.data.map((song) => ndNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)),
                totalRecordCount: Number(res.body.headers.get('x-total-count') || 0),
            };
        };
    ...
    getSongListCount: async ({ apiClientProps, query }) => NavidromeController.getSongList({
        apiClientProps,
        query: { ...query, limit: 1, startIndex: 0 },
    }).then((result) => result.totalRecordCount),
    ...
    getTagList: async (args) => {
        ...
        const tags = Array.from(tagsToValues)
            ...
            options: data[1]
                .sort((a, b) => {
                return numericSortCollator.compare(a.name.toLocaleLowerCase(), b.name.toLocaleLowerCase());
            })
                .map((option) => ({ id: option.id, name: option.name })),
        }))
            .sort((a, b) => collator.compare(a.name.toLocaleLowerCase(), b.name.toLocaleLowerCase()));
    ...
    getTopSongs: async (args) => {
        ...
        if (type === 'community') {
            ...
            return {
                items: (res.body.topSongs?.song || []).map((song) => ssNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)),
                startIndex: 0,
    ...
    replacePlaylist: async (args) => {
        ...
        const existingSongs = existingSongsRes.body.data.map((item) => ndNormalize.song(item, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
    ...
    uploadInternetRadioStationImage: async (args) => {
        const { apiClientProps, body, query } = args;
        const server = apiClientProps.server;
        const serverUrl = server?.url?.replace(/\/$/, '');
        if (!serverUrl) {
    ...
    uploadPlaylistImage: async (args) => {
        const { apiClientProps, body, query } = args;
        const server = apiClientProps.server;
        const serverUrl = server?.url?.replace(/\/$/, '');
        if (!serverUrl) {
