...
import { getFeatures, hasFeature, sortSongList } from '/@/shared/api/utils';
import { albumArtistListSortMap, albumListSortMap, genreListSortMap, LibraryItem, Played, playlistListSortMap, ServerType, SongListSort, songListSortMap, SortOrder, sortOrderMap, } from '/@/shared/types/domain-types';
import { ServerFeature } from '/@/shared/types/features-types';
const getJellyfinImageRequest = ({ apiClientProps: { server }, baseUrl, query, }) => {
    const { id, size } = query;
...
const formatCommaDelimitedString = (value) => {
    return value.join(',');
};
...
export const JellyfinController = {
    ...
    getAlbumArtistInfo: async (args) => {
        ...
        const items = similarArtistsRes.body?.Items?.filter((entry) => entry.Name !== 'Various Artists');
        const similarArtists = items?.map((entry) => ({
            id: entry.Id,
    ...
    getAlbumArtistListCount: async ({ apiClientProps, query }) => JellyfinController.getAlbumArtistList({
        apiClientProps,
    ...
    getAlbumDetail: async (args) => {
        ...
        return jfNormalize.album({ ...res.body, Songs: songsRes.body.Items }, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith);
    },
    ...
    getAlbumListCount: async ({ apiClientProps, query }) => JellyfinController.getAlbumList({
        apiClientProps,
        query: { ...query, limit: 1, startIndex: 0 },
    }).then((result) => result.totalRecordCount),
    getAlbumRadio: async (args) => {
        ...
        return res.body.Items.map((song) => jfNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
    },
    ...
    getArtistListCount: async ({ apiClientProps, query }) => JellyfinController.getArtistList({
        apiClientProps,
        query: { ...query, limit: 1, startIndex: 0 },
    }).then((result) => result.totalRecordCount),
    ...
    getFolder: async (args) => {
        ...
        const folderDetailRes = await jfApiClient(apiClientProps).getFolder({
            ...
            query: {
                ...
                ParentId: query.id,
                SortBy: query.sortBy
                    ? songListSortMap.jellyfin[query.sortBy] || 'SortName'
                    : 'SortName',
        ...
        if (folderDetailRes.body.Items?.length > 0) {
            ...
            if (parentId) {
                ...
                if (parentFolderRes.status === 200) {
                    const parentFolderItem = parentFolderRes.body.Items?.find((item) => item.Id === query.id);
                    if (parentFolderItem) {
        ...
        let filteredSongs = items
            .filter((item) => item.Type === 'Audio' &&
            item.MediaSources)
            .map((item) => jfNormalize.song(item, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
        if (query.searchTerm) {
    ...
    getMusicFolderList: async (args) => {
        ...
        const musicFolders = res.body.Items.filter((folder) => folder.CollectionType === jfType._enum.collection.MUSIC);
        return {
    ...
    getPlaylistListCount: async ({ apiClientProps, query }) => JellyfinController.getPlaylistList({
        apiClientProps,
        query: { ...query, limit: 1, startIndex: 0 },
    }).then((result) => result.totalRecordCount),
    getPlaylistSongList: async (args) => {
        ...
        return {
            items: res.body.Items.map((item) => jfNormalize.song(item, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)),
            startIndex: 0,
            totalRecordCount: res.body.TotalRecordCount,
        };
    },
    ...
    getRandomSongList: async (args) => {
        ...
        const res = await jfApiClient(apiClientProps).getSongList({
            ...
            query: {
                ...
                IncludeItemTypes: 'Audio',
                IsPlayed: query.played === Played.Never
                    ? false
    ...
    getSimilarSongs: async (args) => {
        ...
        if (apiClientProps.server?.preferInstantMix !== true) {
            ...
            if (res.status === 200 && res.body.Items.length) {
                const results = res.body.Items.reduce((acc, song) => {
                    if (song.Id !== query.songId) {
                        acc.push(jfNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
                    }
                    return acc;
                }, []);
                if (results.length > 0) {
        ...
        return mix.body.Items.reduce((acc, song) => {
            if (song.Id !== query.songId) {
                acc.push(jfNormalize.song(song, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
            }
            return acc;
    ...
    getSongDetail: async (args) => {
        ...
        return jfNormalize.song(res.body, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith);
    },
    getSongList: async (args) => {
        ...
        return {
            items: items.map((item) => jfNormalize.song(item, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)),
            startIndex: query.startIndex,
            totalRecordCount,
        };
    },
    getSongListCount: async ({ apiClientProps, query }) => JellyfinController.getSongList({
        apiClientProps,
        query: { ...query, limit: 1, startIndex: 0 },
    }).then((result) => result.totalRecordCount),
    getStreamUrl: async ({ apiClientProps: { server }, query }) => {
        ...
        if (transcode) {
            ...
            url = url.replace('&container=opus,mp3,aac,m4a,m4b,flac,wav,ogg', `&container=${realFormat}`);
    ...
    getTagList: async (args) => {
        ...
        if (res.body.Tags?.length) {
            tags.push({
                ...
                options: res.body.Tags.sort((a, b) => {
                    return numericSortCollator.compare(a.toLocaleLowerCase(), b.toLocaleLowerCase());
        ...
        if (studioRes.body.Items.length) {
            tags.push({
                name: 'Studios',
                options: studioRes.body.Items.sort((a, b) => collator.compare(a.Name.toLocaleLowerCase(), b.Name.toLocaleLowerCase())).map((option) => ({ id: option.Name, name: option.Name })),
            });
        }
    ...
    getTopSongs: async (args) => {
        ...
        const items = res.body.Items.map((item) => jfNormalize.song(item, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
        if (type === 'personal') {
            const sorted = orderBy(items, ['playCount', 'albumId', 'trackNumber'], ['desc', 'asc', 'asc']);
            return {
                items: sorted,
                startIndex: 0,
                totalRecordCount: res.body.TotalRecordCount,
    ...
    replacePlaylist: async (args) => {
        ...
        const existingSongs = existingSongsRes.body.Items.map((item) => jfNormalize.song(item, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
    ...
    search: async (args) => {
        ...
        return {
            ...
            songs: songs.map((item) => jfNormalize.song(item, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)),
        };
    },
