...
import { getDefaultTranscodingProfiles, getDirectPlayProfiles, } from '/@/renderer/features/player/components/audio-players';
...
import { AlbumListSort, GenreListSort, LibraryItem, PlaylistListSort, ServerType, SongListSort, SortOrder, } from '/@/shared/types/domain-types';
import { ServerFeature } from '/@/shared/types/features-types';
const getSubsonicImageRequest = ({ apiClientProps: { server }, baseUrl, query, }) => {
    const { id, size } = query;
...
//     }
//   ];
function appendTranscodeParams(url, format, bitrate) {
    let streamUrl = url;
    if (format) {
...
function buildGetTranscodeStreamUrl(server, args) {
...
export const SubsonicController = {
    ...
    createFavorite: async (args) => {
        ...
        const res = await ssApiClient(apiClientProps).createFavorite({
            query: {
                ...
                artistId: query.type === LibraryItem.ALBUM_ARTIST || query.type === LibraryItem.ARTIST
                    ? query.id
                    : undefined,
    ...
    getAlbumArtistDetail: async (args) => {
        ...
        return {
            ...
            albums: artist.album?.map((album) => ssNormalize.album(album, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)),
            similarArtists: null,
        };
    },
    getAlbumArtistInfo: async (args) => {
        ...
        return {
            ...
            similarArtists: artistInfo?.similarArtist?.map((artist) => ({
                id: artist.id,
    ...
    getAlbumArtistList: async (args) => {
        ...
        let results = artists.map((artist) => ssNormalize.albumArtist(artist, apiClientProps.server));
        if (query.searchTerm) {
    ...
    getAlbumDetail: async (args) => {
        ...
        return ssNormalize.album(res.body.album, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith);
    },
    getAlbumList: async (args) => {
        ...
        if (query.searchTerm) {
            ...
            const results = res.body.searchResult3?.album?.map((album) => ssNormalize.album(album, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)) || [];
            return {
                items: results,
                startIndex: query.startIndex,
        ...
        if (query.artistIds) {
            ...
            for (const artistId of query.artistIds) {
                promises.push(ssApiClient(apiClientProps).getArtist({
                    query: {
                        id: artistId,
                    },
                }));
            ...
            const items = albums.map((album) => ssNormalize.album(album, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith));
            return {
        ...
        if (query.favorite) {
            ...
            const allResults = res.body.starred?.album?.map((album) => ssNormalize.album(album, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)) || [];
        ...
        return {
            items: res.body.albumList2.album?.map((album) => ssNormalize.album(album, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)) || [],
            startIndex: query.startIndex,
            totalRecordCount: null,
        };
    },
    getAlbumListCount: async (args) => {
        ...
        if (query.artistIds) {
            ...
            const artistResult = await Promise.all(promises);
            const albums = artistResult.reduce((total, artist) => {
                if (artist.status !== 200) {
                    return 0;
                }
    ...
    getAlbumRadio: async (args) => {
        ...
        return res.body.similarSongs.song.map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith));
    },
    getArtistList: async (args) => {
        ...
        if (query.role) {
            artists = artists.filter((artist) => !artist.roles || artist.roles.includes(query.role));
        }
        let results = artists.map((artist) => ssNormalize.albumArtist(artist, apiClientProps.server));
        if (query.searchTerm) {
            const searchResults = filter(results, (artist) => {
    ...
    getArtistListCount: async (args) => SubsonicController.getArtistList({
        ...args,
        context: args.context,
        query: { ...args.query, startIndex: 0 },
    }).then((res) => res.totalRecordCount),
    getArtistRadio: async (args) => {
        ...
        return res.body.similarSongs2.song.map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith));
    },
    getDownloadUrl: (args) => {
        const { apiClientProps, query } = args;
        return (`${apiClientProps.server?.url}/rest/download.view` +
            `?id=${query.id}` +
    ...
    getFolder: async ({ apiClientProps, context, query }) => {
        const sortOrder = (query.sortOrder?.toLowerCase() ?? 'asc');
        ...
        if (isRootFolderId) {
            ...
            let items = res.body.indexes?.index?.flatMap((idx) => idx.artist.map((artist) => ({
                artist: artist.name,
            ...
            let folders = items.map((item) => ssNormalize.folder(item, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith));
        ...
        const folder = ssNormalize.folder(directoryRes.body.directory, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith);
        ...
        if (query.searchTerm) {
            ...
            filteredFolders = filter(filteredFolders, (f) => f.name.toLowerCase().includes(searchTermLower));
        ...
        if (filteredSongs.length > 0) {
            filteredSongs = sortSongList(filteredSongs, query.sortBy || SongListSort.NAME, query.sortOrder || SortOrder.ASC);
        }
        return {
            ...folder,
            children: {
    ...
    getGenreList: async ({ apiClientProps, query }) => {
        ...
        if (query.searchTerm) {
            const searchResults = filter(results, (genre) => genre.value.toLowerCase().includes(query.searchTerm.toLowerCase()));
            results = searchResults;
        }
        switch (query.sortBy) {
    ...
    getPlaylistList: async ({ apiClientProps, query }) => {
        const sortOrder = (query.sortOrder || SortOrder.ASC).toLowerCase();
        ...
        const playlists = results.map((playlist) => ssNormalize.playlist(playlist, apiClientProps.server));
    ...
    getPlaylistSongList: async ({ apiClientProps, context, query }) => {
        ...
        const items = res.body.playlist.entry?.map((song, index) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith, index)) || [];
        return {
            items,
            startIndex: 0,
    ...
    getPlayQueue: async ({ apiClientProps, context }) => {
        if (hasFeature(apiClientProps.server, ServerFeature.SERVER_PLAY_QUEUE)) {
            ...
            const { changed, changedBy, currentIndex, entry, position, username } = res.body.playQueueByIndex || {}; // if there is no queue saved, playQueueByIndex may be undefined from a bug in Navidrome
            return {
                changed: changed ?? '',
                changedBy: changedBy ?? '',
                currentIndex: currentIndex ?? 0,
                entry: entry?.map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith)) || [],
    ...
    getRandomSongList: async (args) => {
        ...
        const normalizedResults = results.map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith));
        return {
    ...
    getRoles: async (args) => {
        ...
        const final = Array.from(roles).sort();
    ...
    getSimilarSongs: async (args) => {
        ...
        return res.body.similarSongs.song.reduce((acc, song) => {
            if (song.id !== query.songId) {
                acc.push(ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith));
            }
            return acc;
        }, []);
    },
    getSongDetail: async (args) => {
        ...
        return ssNormalize.song(res.body.song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith);
    },
    getSongList: async ({ apiClientProps, context, query }) => {
        ...
        if (query.searchTerm) {
            ...
            return {
                items: res.body.searchResult3?.song?.map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith)) || [],
                startIndex: query.startIndex,
                totalRecordCount: null,
            };
        }
        if (query.genreIds) {
            ...
            return {
                items: results.map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith)) || [],
                startIndex: 0,
                totalRecordCount: null,
            };
        }
        if (query.favorite) {
            ...
            let allResults = (res.body.starred?.song || []).map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith)) || [];
            const filterArtistIds = query.albumArtistIds || query.artistIds;
            if (filterArtistIds?.length) {
                const idSet = new Set(filterArtistIds);
                allResults = allResults.filter((song) => song.albumArtists?.some((aa) => idSet.has(aa.id)));
            }
            return sortAndPaginate(allResults, {
                limit: query.limit,
                sortBy: query.sortBy,
        ...
        if (query.albumIds || artistIds) {
            if (query.albumIds) {
                for (const albumId of query.albumIds) {
                    fromAlbumPromises.push(ssApiClient(apiClientProps).getAlbum({
                        query: {
                            id: albumId,
                        },
                    }));
            ...
            if (artistIds) {
                for (const artistId of artistIds) {
                    artistDetailPromises.push(ssApiClient(apiClientProps).getArtist({
                        query: {
                            id: artistId,
                        },
                    }));
    ...
    getStreamUrl: async ({ apiClientProps, query }) => {
        ...
        const streamUrl = `${server?.url}/rest/stream.view?id=${id}&v=1.13.0&c=Samo&${server?.credential}`;
        ...
        if (hasFeature(server, ServerFeature.OS_TRANSCODE_DECISION)) {
            ...
            if (transcodeDecision.status !== 200) {
                logFn.error(`Failed to get transcode decision for song ${id}, falling back to direct stream`);
                return streamUrl;
            }
    ...
    getTopSongs: async (args) => {
        ...
        if (type === 'community') {
            ...
            return {
                items: (res.body.topSongs?.song || []).map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith)),
                startIndex: 0,
        ...
        const songsWithPlayCount = orderBy(res.items.filter((song) => song.playCount > 0), ['playCount', 'albumId', 'trackNumber'], ['desc', 'asc', 'asc']);
        return {
    ...
    replacePlaylist: async (args) => {
        ...
        const existingSongs = existingSongsRes.body.playlist.entry?.map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith)) || [];
        ...
        const playlist = ssNormalize.playlist(playlistDetailRes.body.playlist, apiClientProps.server);
    ...
    savePlayQueue: async ({ apiClientProps, query }) => {
        if (hasFeature(apiClientProps.server, ServerFeature.SERVER_PLAY_QUEUE)) {
            ...
            const res = await ssApiClient(apiClientProps).savePlayQueue({
                query: {
                    current: query.currentIndex !== undefined && query.currentIndex < query.songs.length
    ...
    search: async (args) => {
        ...
        return {
            albumArtists: (res.body.searchResult3?.artist || [])?.map((artist) => ssNormalize.albumArtist(artist, apiClientProps.server)),
            albums: (res.body.searchResult3?.album || []).map((album) => ssNormalize.album(album, apiClientProps.server, args.context?.pathReplace, args.context?.pathReplaceWith)),
            songs: (res.body.searchResult3?.song || []).map((song) => ssNormalize.song(song, apiClientProps.server, context?.pathReplace, context?.pathReplaceWith)),
        };
    },
...
function getLibraryId(musicFolderId) {
