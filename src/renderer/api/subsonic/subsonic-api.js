    ...
    if (data['subsonic-response'].status !== 'ok') {
        ...
        if (data['subsonic-response'].error.code !== 0) {
            toast.error({
                ...
                title: i18n.t('error.genericError', { postProcess: 'sentenceCase' }),
            });
...
const silentlyTransformResponse = (data) => {
...
export const ssApiClient = (args) => {
    ...
    return initClient(contract, {
        api: async ({ body, headers, method, path, rawQuery }) => {
            ...
            const isGetTranscodeDecisionPost = method === 'POST' && api === 'getTranscodeDecision.view';
