...
export function useSongUrl(song, current, transcode, enabled = true) {
    const prior = useRef(['', '']);
    const shouldReusePrior = Boolean(song?._serverId && current && prior.current[0] === song._uniqueId && prior.current[1]);
    const { data: queryStreamUrl } = useQuery({
        enabled: enabled && Boolean(song?._serverId) && !shouldReusePrior,
        queryFn: () => api.controller.getStreamUrl({
...
export const getSongUrl = async (song, transcode, skipAutoTranscode) => {
