...
import { registerAudioElement, stopAudioElement, unregisterAudioElement, warnIfMultipleAudiblePlaybackElements, } from '/@/renderer/features/player/audio-player/audio-element-registry';
...
const EMPTY_SOURCE = 'data:audio/mp3;base64,';
export const WebPlayerEngine = (props) => {
    const { isMuted, isTransitioning, onEndedPlayer1, onEndedPlayer2, onErrorPause, onProgressPlayer1, onProgressPlayer2, onStartedPlayer1, onStartedPlayer2, playerNum, playerRef, playerStatus, preservesPitch, speed, src1, src2, volume, } = props;
    const playbackSession = usePlaybackSession();
    const player1Ref = useRef(null);
    const player2Ref = useRef(null);
    const ownedAudioElementsRef = useRef(new Set());
    ...
    const networkRetryTimeout1 = useRef(null);
    const networkRetryTimeout2 = useRef(null);
    const [ReactPlayerComponent, setReactPlayerComponent] = useState(null);
    ...
    const clearAudioElement = useCallback((player) => {
    ...
    useImperativeHandle(playerRef, () => ({
        ...
        player1() {
            return {
                ...
                setVolume: (volume) => setInternalVolume1(volume / 100 || 0),
            };
        },
        player2() {
            return {
                ref: player2Ref?.current,
                setVolume: (volume) => setInternalVolume2(volume / 100 || 0),
            };
        },
        seekTo(seekTo) {
            playerNum === 1
    ...
    const registerPlayerAudioElement = useCallback((player, playerId, mediaKey) => {
        const internal = player?.getInternalPlayer();
        if (!(internal instanceof HTMLAudioElement))
            return null;
    ...
    const handleOnError = (playerRef, playerId, sourceRef, onEnded, onErrorPause, networkRetryCountRef, retryTimeoutRef) => {
        return ({ target }) => {
            ...
            if (isNetworkError) {
                if (networkRetryCountRef.current < MAX_NETWORK_RETRIES) {
                    ...
                    retryTimeoutRef.current = setTimeout(() => {
                        ...
                        if (playbackSessionIdRef.current !== scheduledSessionId ||
    ...
    const handleOnReadyPlayer1 = useCallback((player) => {
    ...
    const handleOnReadyPlayer2 = useCallback((player) => {
    ...
    if (isLoading || !ReactPlayerComponent) {
        return _jsx("div", { id: "web-player-engine", style: { display: 'none' } });
    }
    return (_jsxs("div", { id: "web-player-engine", style: { display: 'none' }, children: [_jsx(ReactPlayerComponent, { config: {
                    file: { attributes: { crossOrigin: 'anonymous' }, forceAudio: true },
                }, controls: false, height: 0, id: "web-player-1", muted: isMuted, onEnded: src1 ? () => onEndedPlayer1() : undefined, onError: handleOnError(player1Ref, 'web-player-1', src1Ref, () => onEndedPlayer1(), onErrorPause, networkRetryCount1, networkRetryTimeout1), onPlay: warnIfMultipleAudiblePlaybackElements, onProgress: onProgressPlayer1, onReady: handleOnReadyPlayer1, playbackRate: speed || 1, playing: playerNum === 1 && playerStatus === PlayerStatus.PLAYING, progressInterval: isTransitioning ? 10 : 250, ref: player1Ref, url: src1 || EMPTY_SOURCE, volume: volume1, width: 0 }), _jsx(ReactPlayerComponent, { config: {
                    file: { attributes: { crossOrigin: 'anonymous' }, forceAudio: true },
                }, controls: false, height: 0, id: "web-player-2", muted: isMuted, onEnded: src2 ? () => onEndedPlayer2() : undefined, onError: handleOnError(player2Ref, 'web-player-2', src2Ref, () => onEndedPlayer2(), onErrorPause, networkRetryCount2, networkRetryTimeout2), onPlay: warnIfMultipleAudiblePlaybackElements, onProgress: onProgressPlayer2, onReady: handleOnReadyPlayer2, playbackRate: speed || 1, playing: playerNum === 2 && playerStatus === PlayerStatus.PLAYING, progressInterval: isTransitioning ? 10 : 250, ref: player2Ref, url: src2 || EMPTY_SOURCE, volume: volume2, width: 0 })] }));
};
