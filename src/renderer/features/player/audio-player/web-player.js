...
import { useMpvSettings, usePlaybackSettings, usePlayerActions, usePlayerData, usePlayerProperties, usePlayerStoreBase, usePlayerVolumeState, } from '/@/renderer/store';
...
export function WebPlayer() {
    ...
    const [player2Source, setPlayer2Source] = useState(null);
    const fadeAndSetStatus = useCallback(async (startVolume, endVolume, duration, status) => {
    ...
    const onProgressPlayer1 = useCallback((e) => {
    ...
    const onProgressPlayer2 = useCallback((e) => {
    ...
    useEffect(() => {
        ...
        const tick = () => {
            const activePlayer = num === 1 ? playerRef.current?.player1() : playerRef.current?.player2();
            const internalPlayer = activePlayer?.ref?.getInternalPlayer();
            if (internalPlayer) {
    ...
    const calculateReplayGain = useCallback((song) => {
    ...
    const handlePlayer1Start = useCallback(async (player) => {
    ...
    const handlePlayer2Start = useCallback(async (player) => {
    ...
    return (_jsx(WebPlayerEngine, { isMuted: isMuted, isTransitioning: Boolean(isTransitioning), onEndedPlayer1: handleOnEndedPlayer1, onEndedPlayer2: handleOnEndedPlayer2, onErrorPause: handleOnErrorPause, onProgressPlayer1: onProgressPlayer1, onProgressPlayer2: onProgressPlayer2, onStartedPlayer1: handlePlayer1Start, onStartedPlayer2: handlePlayer2Start, playerNum: num, playerRef: playerRef, playerStatus: localPlayerStatus, preservesPitch: preservePitch, speed: speed, src1: player1Url, src2: player2Url, volume: volume }));
}
function crossfadeHandler(args) {
    const { crossfadeDuration, crossfadeStyle, currentPlayer, currentPlayerNum, currentTime, duration, hasNextSong, isTransitioning, nextPlayer, playerNum, setIsTransitioning, volume, } = args;
 ...
 */
function equalPowerEaseIn(t) {
    const clampedT = Math.max(0, Math.min(1, t));
    return Math.sin((Math.PI / 2) * clampedT);
}
function equalPowerEaseOut(t) {
    const clampedT = Math.max(0, Math.min(1, t));
...
function getCrossfadeEasing(style) {
    switch (style) {
...
function getDuration(ref) {
    return ref?.getInternalPlayer()?.duration || 0;
}
function getDurationPadding(isFlac) {
    switch (isFlac) {
        case false:
            return 0.116;
        case true:
