...
type MpvPlaybackSettings = ReturnType<typeof usePlaybackSettings>;

const getMpvProperties = (
    playback: MpvPlaybackSettings,
    muted: boolean,
    speed: number,
): Record<string, any> => {
    const {
...
const getDirectSongUrl = (song?: QueueSong) => {
    if (!song) {
...
export const MpvPlayer = () => {
    ...
    const nextSongIdRef = useRef<string | undefined>(undefined);
    const queueSyncEpochRef = useRef(0);
    const statusRef = useRef(status);
    const mediaAutoNextRef = useRef(mediaAutoNext);

    const mpvProperties = useMemo(
        () => getMpvProperties(playback, muted, speed),
        [muted, playback, speed],
    );
    ...
    useEffect(() => {
        ...
        const initialize = async () => {
            try {
                ...
                toast.error({
                    message:
                        'Native playback failed to start. Web compatibility is available in playback settings, but it will use the browser audio engine.',
    ...
    useEffect(() => {
                ...
                if (nextCurrentSong?._uniqueId && nextCurrentSong._uniqueId === previousSongId) {
                    ...
                    if (repeatedUrl) {
                        ...
                        await mpvPlayer.setQueue(
                            repeatedUrl,
                            followingUrl,
                            nextPlayerData.status !== PlayerStatus.PLAYING,
                        );
                    }
                } else {
                    currentSongIdRef.current = nextCurrentSong?._uniqueId;
        ...
        const handleFallback = (_event: any, isError: boolean) => {
            if (!isError) {
                return;
            }
            ...
            toast.error({
                message:
                    'Native playback failed. Switch to Web compatibility in playback settings only if you accept browser-engine playback.',
                title: 'Native playback unavailable',
            });
        };
