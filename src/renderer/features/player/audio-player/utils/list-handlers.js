export const gaplessHandler = (args) => {
    const { currentTime, duration, isFlac, isTransitioning, nextPlayerRef, setIsTransitioning } = args;
    if (!isTransitioning) {
...
export const crossfadeHandler = (args) => {
    const { currentPlayer, currentPlayerRef, currentTime, duration, fadeDuration, fadeType, isTransitioning, nextPlayerRef, player, setIsTransitioning, volume, } = args;
    ...
    const currentPlayerVolume = currentPlayerVolumeCalculation >= 0 ? currentPlayerVolumeCalculation : 0;
    const nextPlayerVolume = nextPlayerVolumeCalculation <= volume ? nextPlayerVolumeCalculation : volume;
    if (currentPlayer === 1) {
