...
import { BarAlign, usePlayerbarSlider, usePlayerSong, usePlayerStatus, usePlayerTimestamp, } from '/@/renderer/store';
...
export const PlayerbarWaveform = () => {
    ...
    const audioElementRef = useRef(createWaveformPreviewAudioElement());
    ...
    const containerPositionRef = useRef(null);
    const songDuration = currentSong?.duration ? currentSong.duration / 1000 : 0;
    const streamUrl = useSongUrl(currentSong, true, { bitrate: 64, enabled: false, format: 'mp3' }, playerStatus === PlayerStatus.PLAYING);
    const { color } = useAppThemeColors();
    const primaryColor = color['--theme-colors-primary'] || 'rgb(53, 116, 252)';
    ...
    const { wavesurfer } = useWavesurfer({
        barAlign: playerbarSlider?.barAlign === BarAlign.CENTER ? undefined : playerbarSlider?.barAlign,
    ...
    useEffect(() => {
        if (!wavesurfer || !songDuration || !containerRef.current)
            return;
    ...
    useEffect(() => {
        if (!wavesurfer || !songDuration || isDragging)
            return;
        const duration = wavesurfer.getDuration();
    ...
    if (!currentSong) {
        return (_jsx(CustomPlayerbarSlider, { disabled: true, max: 100, min: 0, onClick: (e) => {
                e?.stopPropagation();
            }, size: 6, value: 0, w: "100%" }));
    }
    return (_jsxs("div", { className: styles.wavesurferContainer, onClick: (e) => {
            e?.stopPropagation();
        }, style: { position: 'relative' }, children: [_jsx(motion.div, { animate: { opacity: isLoading ? 0 : 1 }, className: styles.waveform, initial: { opacity: 0 }, ref: containerRef, transition: { duration: 0.2 } }), _jsx(AnimatePresence, { children: isLoading && (_jsx(motion.div, { animate: { opacity: 1 }, exit: { opacity: 0 }, initial: { opacity: 0 }, style: {
                        height: '100%',
                        left: 0,
                        position: 'absolute',
                        top: 3,
                        width: '100%',
                    }, transition: { duration: 0.2 }, children: _jsx(PlayerbarSeekSlider, { max: songDuration, min: 0 }) })) }), tooltipPosition && isDragging && (_jsx(motion.div, { animate: { opacity: 1, scale: 1, x: '-50%' }, className: styles.tooltip, initial: { opacity: 0, scale: 0.8, x: '-50%' }, style: {
                    left: `${tooltipPosition.x}px`,
                    position: 'fixed',
                    top: `${tooltipPosition.y - 40}px`,
                    zIndex: 1000,
                }, transition: { duration: 0.15 }, children: _jsx(Text, { isNoSelect: true, size: "md", children: formatDuration(tooltipValue * 1000) }) }))] }));
};
