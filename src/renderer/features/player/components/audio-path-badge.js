...
export const AudioPathBadge = ({ compact = false, inline = false, mode = 'detail', song, }) => {
    ...
    if (mode === 'playerbar') {
        return (_jsx(Group, { gap: inline ? 3 : 4, justify: "flex-start", wrap: "nowrap", children: items.map((item, index) => (_jsx(Badge, { radius: inline ? 'xs' : 'sm', size: "xs", styles: getToneStyles(item.tone), variant: item.tone === 'neutral' ? 'light' : 'outline', children: item.label }, `${item.label}-${index}`))) }));
    }
    return (_jsx(Group, { gap: inline ? 3 : compact ? 4 : 'xs', justify: compact || inline ? 'flex-start' : 'center', wrap: "nowrap", children: items.map((item, index) => (_jsx(Badge, { radius: inline ? 'xs' : 'sm', size: compact || inline ? 'xs' : 'lg', styles: getToneStyles(item.tone), variant: item.tone === 'neutral' ? 'light' : 'outline', children: item.label }, `${item.label}-${index}`))) }));
};
