...
import { isShuffleEnabled, mapShuffledToQueueIndex, useAutoDJSettings, useCurrentServer, useCurrentServerId, getQueue, usePlayerStoreBase, useSettingsStore, } from '/@/renderer/store';
...
export const useAutoDJ = () => {
    ...
    useEffect(() => {
        const unsubscribe = usePlayerStoreBase.subscribe((state) => {
            ...
            try {
                ...
                let uniqueSimilarSongs = [];
                const hasMusicFolder = server?.musicFolderId && server.musicFolderId.length > 0;
                const trySimilarSongs = !hasMusicFolder || (hasMusicFolder && hasSimilarSongsMusicFolder);
                ...
                if (trySimilarSongs) {
                    ...
                    uniqueSimilarSongs = similarSongs.filter((song) => !queueSongIdSet.has(song.id));
                }
                ...
                if (uniqueSimilarSongs.length < settings.itemCount) {
                    ...
                    if (genre) {
                        ...
                        const genreSongs = genreSimilarSongs.items.filter((song) => !queueSongIdSet.has(song.id));
                        ...
                        if (!trySimilarSongs) {
                            ...
                            const uniqueRandomSongs = randomSongs.items.filter((song) => !queueSongIdSet.has(song.id));
                ...
                if (uniqueSimilarSongs.length < settings.itemCount) {
                    ...
                    if (albumArtist) {
                        ...
                        uniqueSimilarSongs.push(...albumArtistSimilarSongs.items.filter((song) => !queueSongIdSet.has(song.id)));
                    }
                }
                ...
                if (uniqueSimilarSongs.length < settings.itemCount) {
                    ...
                    uniqueSimilarSongs.push(...randomSongs.items.filter((song) => !queueSongIdSet.has(song.id)));
                }
                ...
                if (stateAfter.player.status === PlayerStatus.PAUSED &&
                ...
                logFn.error(logMsg[LogCategory.PLAYER].autoPlayFailed, {
                    category: LogCategory.PLAYER,
                    meta: { error: error.message, songId: properties.song?.id },
                });
            }
        }, {
            equalityFn: (a, b) => {
