...
import { setTimestamp, useCurrentServerId, getQueue, usePlayerStore, useTimestampStoreBase, } from '/@/renderer/store';
    ...
    const mutation = useMutation({
        mutationFn: async () => {
            ...
            if (queue.items.some((item) => item._serverId !== serverId)) {
                ...
                throw new Error(`${t('error.multipleServerSaveQueueError', { postProcess: 'sentenceCase' })}`);
            }
            try {
...
export const useRestoreQueue = () => {
    ...
    const handleRestoreQueue = useCallback(async () => {
        if (!serverId)
            return;
        try {
            const queue = await queryClient.fetchQuery(songsQueries.getQueue({ query: {}, serverId }));
            if (queue) {
                player.setQueue(queue.entry, queue.currentIndex, queue.positionMs !== undefined ? queue.positionMs / 1000 : undefined);
            }
        }
        catch (error) {
            toast.error({
