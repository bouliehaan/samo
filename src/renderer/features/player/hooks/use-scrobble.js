...
import { useAppStore, useOfflineMode, usePlaybackOwnerStore, usePlaybackSettings, getCurrentSong, usePlayerSong, usePlayerStore, useSettingsStore, useTimestampStoreBase, } from '/@/renderer/store';
...
const checkScrobbleConditions = (args) => {
    const { scrobbleAtDurationMs, scrobbleAtPercentage, songCompletedDurationMs, songDurationMs } = args;
...
const getSongSessionKey = (song) => `${song._uniqueId}:${song.id}`;
...
export const useScrobble = () => {
    ...
    const lastSeekEventRef = useRef(0);
    const scrobbledSessionKeyRef = useRef(undefined);
    const songChangeTimeoutRef = useRef(undefined);
    const notifyTimeoutRef = useRef(undefined);
    useEffect(() => {
        imageUrlRef.current = imageUrl;
    }, [imageUrl]);
    const handleScrobbleFromProgress = useCallback((properties, prev) => {
        if (!isScrobbleEnabled || isPrivateModeEnabled || !isMusicPlaybackSource())
            return;
        const currentSong = getCurrentSong();
        const currentStatus = usePlayerStore.getState().player.status;
        if (!currentSong?.id || currentStatus !== PlayerStatus.PLAYING)
            return;
        ...
        if (!isCurrentSongScrobbled &&
            ...
            if (shouldSubmitScrobble) {
                ...
                const position = currentSong._serverType === ServerType.JELLYFIN
    ...
    const handleScrobbleFromSongChange = useCallback((properties, prev) => {
        ...
        if (scrobbleSettings?.notify && currentSong?.id) {
            ...
            notifyTimeoutRef.current = setTimeout(() => {
                if (currentSong._uniqueId !== previousSong?._uniqueId ||
                    properties.index !== prev.index) {
                    const artists = currentSong.artists?.length > 0
    ...
    const handleScrobbleFromStatus = useCallback((properties, prev) => {
        if (!isScrobbleEnabled || isPrivateModeEnabled || !isMusicPlaybackSource()) {
            return;
        }
        const currentSong = getCurrentSong();
    ...
    const handleProgressUpdate = useCallback((properties, prev) => {
