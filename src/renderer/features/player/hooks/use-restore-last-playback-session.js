...
import { isStructuredMusicContext, rememberMusicPlaybackSession, SONG_CONTEXT, useLastPlaybackSessionStore, } from '/@/renderer/store/last-playback-session.store';
import { usePlaybackOwnerStore } from '/@/renderer/store/playback-owner.store';
import { getCurrentSong, getQueue, usePlayerHydrated, usePlayerStoreBase, } from '/@/renderer/store/player.store';
...
const restoreAudiobookSession = async (session) => {
    const server = getServerById(session.serverId);
    if (!server)
        return false;
    const item = await audiobookshelfController.getItem(server, session.itemId);
    if (usePlaybackOwnerStore.getState().source)
        return true;
    const position = session.position ?? useAudiobookStore.getState().resumeByItemId[session.itemId] ?? 0;
...
const restorePodcastSession = async (session) => {
    const server = getServerById(session.serverId);
    if (!server)
        return false;
    const item = await audiobookshelfController.getItem(server, session.itemId);
    ...
    const position = session.position ?? usePodcastStore.getState().resumeByEpisodeKey[resumeKey] ?? 0;
    const clampedPosition = duration > 0 ? Math.min(Math.max(0, position), duration) : Math.max(0, position);
...
const restoreMusicSession = async (session, queryClient) => {
    if (usePlaybackOwnerStore.getState().source)
        return true;
    ...
    if (queueAlreadyRestored && isStructuredMusicContext(restoredQueueContext)) {
        ...
        rememberMusicPlaybackSession({
            ...
            songRef: song?.id && song?._serverId
...
const restoreRadioSession = (session) => {
