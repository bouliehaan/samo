...
import { subscribePlayerStatus, usePlaybackSettings, useSettingsStore, useSkipButtons, useTimestampStoreBase, } from '/@/renderer/store';
...
const clampSeekPosition = (target) => {
    const source = usePlaybackOwnerStore.getState().source;
    const duration = source === 'audiobook'
...
export const useMediaSession = () => {
    ...
    useEffect(() => {
        ...
        mediaSession.setActionHandler('seekto', (e) => {
            if (!getNowPlayingSnapshot().canSeek)
                return;
            if (typeof e.seekTime === 'number') {
                playerRef.current.mediaSeekToTimestamp(clampSeekPosition(e.seekTime));
            }
            else if (e.seekOffset) {
                playerRef.current.mediaSeekToTimestamp(clampSeekPosition(getSeekPosition() + e.seekOffset));
            }
        });
        ...
        mediaSession.setActionHandler('seekbackward', (e) => {
            if (!getNowPlayingSnapshot().canSeek)
                return;
            playerRef.current.mediaSeekToTimestamp(clampSeekPosition(getSeekPosition() - (e.seekOffset || skipRef.current?.skipBackwardSeconds || 5)));
        });
        mediaSession.setActionHandler('seekforward', (e) => {
            if (!getNowPlayingSnapshot().canSeek)
                return;
            playerRef.current.mediaSeekToTimestamp(clampSeekPosition(getSeekPosition() + (e.seekOffset || skipRef.current?.skipForwardSeconds || 5)));
        });
        return () => {
            mediaSession.setActionHandler('nexttrack', null);
            mediaSession.setActionHandler('pause', null);
    ...
    const debouncedUpdateMetadata = useRef(debounce(() => {
    ...
    useEffect(() => {
        const unsubscribeSource = usePlaybackOwnerStore.subscribe((state) => state.source, () => {
            if (!isMediaSessionEnabledRef.current)
                return;
            debouncedUpdateMetadata();
        });
        ...
        const unsubscribePodcast = usePodcastStore.subscribe(() => {
            if (!isMediaSessionEnabledRef.current)
                return;
            if (usePlaybackOwnerStore.getState().source !== 'podcast')
                return;
            debouncedUpdateMetadata();
        });
