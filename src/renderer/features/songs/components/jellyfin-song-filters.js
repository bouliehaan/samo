...
export const JellyfinSongFilters = ({ disableArtistFilter, disableGenreFilter, }) => {
    const server = useCurrentServer();
    const serverId = server.id;
    const { t } = useTranslation();
    const { query, setArtistIds, setCustom, setFavorite, setMaxYear, setMinYear } = useSongListFilters();
    ...
    return (_jsxs(Stack, { px: "md", py: "md", children: [yesNoFilters.map((filter) => (_jsx(YesNoSelect, { label: filter.label, onChange: (e) => filter.onChange(e ? e === 'true' : undefined), value: filter.value ? filter.value.toString() : undefined }, `jf-filter-${filter.label}`))), !disableArtistFilter && (_jsxs(_Fragment, { children: [_jsx(Divider, { my: "md" }), _jsx(VirtualMultiSelect, { displayCountType: "song", height: 300, label: artistFilterLabel, onChange: handleArtistChange, options: selectableAlbumArtists, RowComponent: ArtistMultiSelectRow, singleSelect: artistSelectMode === 'single', value: selectedArtistIds })] })), !disableGenreFilter && (_jsxs(_Fragment, { children: [_jsx(Divider, { my: "md" }), _jsx(VirtualMultiSelect, { displayCountType: "song", height: 220, isLoading: genreListQuery.isFetching, label: genreFilterLabel, onChange: handleGenresFilter, options: genreList, RowComponent: GenreMultiSelectRow, singleSelect: genreSelectMode === 'single', value: selectedGenres })] })), _jsx(Divider, { my: "md" }), _jsxs(Group, { grow: true, children: [_jsx(NumberInput, { hideControls: false, label: t('filter.fromYear', { postProcess: 'sentenceCase' }), max: 2300, min: 1700, onChange: (e) => debouncedHandleMinYearFilter(e), required: !!query.minYear, value: query.minYear ?? undefined }), _jsx(NumberInput, { hideControls: false, label: t('filter.toYear', { postProcess: 'sentenceCase' }), max: 2300, min: 1700, onChange: (e) => debouncedHandleMaxYearFilter(e), required: !!query.minYear, value: query.maxYear ?? undefined })] }), _jsx(TagFilters, { query: query, setCustom: setCustom, type: LibraryItem.SONG })] }));
};
