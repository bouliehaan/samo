...
import { List } from 'react-window-v2';
...
export const FolderTreeBrowser = ({ fetchFolder, rootFolderQuery }) => {
    const { currentFolderId, folderPath, setFolderPath } = useFolderListFilters();
    const [expandedNodes, setExpandedNodes] = useState(new Set());
    const [loadedNodes, setLoadedNodes] = useState(new Map());
    const containerRef = useRef(null);
    const previousFolderPathRef = useRef([]);
    const lastInternalFolderPathRef = useRef(null);
    ...
    const fetchFolderChildren = useCallback(async (folderId) => {
    ...
    const buildTree = useCallback((folder, depth = 0) => {
        const folderId = folder.id;
        const isExpanded = expandedNodes.has(folderId);
        const children = getFolderChildren(folder);
        const hasChildren = children.length > 0;
        const childrenLoaded = loadedNodes.has(folderId) || (folder.children?.folders?.length ?? 0) > 0;
        return {
            childrenLoaded,
            depth,
            folder,
    ...
    const flattenedNodes = useMemo(() => {
        ...
        const traverse = (folder, depth, path = []) => {
    ...
    const toggleNode = useCallback((folderId, hasChildren, folder) => {
    ...
    const expandNode = useCallback((folderId, hasChildren, folder) => {
        setExpandedNodes((prev) => {
    ...
    const handleNodeClick = useCallback((folder, path, isExpanded, isCurrentFolder) => {
    ...
    const [shouldScrollToFolder, setShouldScrollToFolder] = useState(null);
    // Handle external navigation - expand parent folders
    useEffect(() => {
        // Skip if folderPath hasn't actually changed
        const pathChanged = previousFolderPathRef.current.length !== folderPath.length ||
        ...
        const isInternalNavigation = lastInternalFolderPathRef.current &&
            lastInternalFolderPathRef.current.length === folderPath.length &&
            lastInternalFolderPathRef.current.every((item, index) => item.id === folderPath[index]?.id);
    ...
    useEffect(() => {
        ...
        const currentIndex = flattenedNodes.findIndex((node) => node.folder.id === shouldScrollToFolder);
        if (currentIndex !== -1) {
            const viewport = containerRef.current.firstElementChild;
            if (viewport) {
    ...
    return (_jsx("div", { className: styles.container, ref: containerRef, children: _jsx(List, { rowComponent: RowComponent, rowCount: flattenedNodes.length, rowHeight: ITEM_HEIGHT, rowProps: rowProps }) }));
};
const RowComponent = ({ currentFolderId, data, handleNodeClick, index, style, toggleNode, }) => {
    ...
    const { isDragging, ref: dragRef } = useDragDrop({
        drag: {
    ...
    const calculateOffset = useCallback(() => {
        ...
        if (rowElement && folderIconRef.current && expandIconRef.current) {
            ...
            const itemPadding = 8;
            setTooltipOffset(-width + paddingLeft + folderIconWidth + expandIconWidth + itemPadding);
        }
    }, [item.depth]);
    useLayoutEffect(() => {
        if (!item) {
    ...
    return (_jsx(Tooltip, { classNames: {
            tooltip: styles.tooltip,
        }, label: item.folder.name, offset: tooltipOffset, openDelay: 0, position: "right", withArrow: false, children: _jsx("div", { className: clsx(styles.row, {
                [styles.active]: isActive,
                [styles.dragging]: isDragging,
            }), onClick: handleRowClick, onContextMenu: handleContextMenu, ref: mergedRef, style: {
                ...style,
                paddingLeft: `${paddingLeft}px`,
            }, children: _jsxs("div", { className: styles.rowContent, children: [item.hasChildren ? (_jsx("div", { className: styles.expandIconContainer, ref: expandIconRef, children: _jsx(Icon, { className: clsx(styles.expandIcon, {
                                [styles.expanded]: item.isExpanded,
                            }), icon: "arrowRightS", onClick: handleExpandClick, size: "sm" }) })) : (_jsx("div", { className: styles.expandIconPlaceholder, ref: expandIconRef })), _jsx("div", { className: styles.folderIconContainer, ref: folderIconRef, children: _jsx(Icon, { className: styles.folderIcon, icon: "folder", size: "md" }) }), _jsx("span", { className: styles.folderName, ref: folderNameRef, children: item.folder.name })] }) }) }));
};
