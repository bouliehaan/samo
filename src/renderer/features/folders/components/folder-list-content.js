...
export const FolderListContent = () => {
    return (_jsx(Suspense, { fallback: _jsx(Spinner, { container: true }), children: _jsx(FolderListInnerContent, {}) }));
};
export const FolderListInnerContent = () => {
    const serverId = useCurrentServerId();
    const queryClient = useQueryClient();
    const { currentFolderId, query } = useFolderListFilters();
    const getFolderQueryOptions = useCallback((folderId) => {
        return folderQueries.folder({
            query: {
                id: folderId,
                searchTerm: query[FILTER_KEYS.SHARED.SEARCH_TERM],
                sortBy: query[FILTER_KEYS.SHARED.SORT_BY] || SongListSort.NAME,
                sortOrder: query[FILTER_KEYS.SHARED.SORT_ORDER] || SortOrder.ASC,
            },
            serverId,
        });
    }, [serverId, query]);
    ...
    return (_jsxs(_Fragment, { children: [_jsx(ListWithSidebarContainer.SidebarPortal, { children: _jsx(FolderTreeBrowser, { fetchFolder: fetchFolder, rootFolderQuery: rootFolderQuery }) }), _jsx(FolderListView, { folderQuery: currentFolderQuery })] }));
};
export const FolderListView = ({ folderQuery }) => {
    ...
    const overrideControls = useMemo(() => {
        return {
            onDoubleClick: ({ internalState, item, meta }) => {
                if (!item) {
                    return;
                }
                if (item._itemType === LibraryItem.FOLDER) {
                    const folder = item;
                ...
                const validSongs = data.filter((d) => {
                    return (d._itemType === LibraryItem.SONG);
                });
    ...
    switch (display) {
        ...
        case ListDisplayType.TABLE: {
            return (_jsx(ItemTableList, { activeRowId: currentSong?.id, autoFitColumns: table.autoFitColumns, CellComponent: ItemTableListColumn, columns: table.columns, data: allItems, enableAlternateRowColors: table.enableAlternateRowColors, enableDrag: true, enableExpansion: false, enableHeader: table.enableHeader, enableHorizontalBorders: table.enableHorizontalBorders, enableRowHoverHighlight: table.enableRowHoverHighlight, enableVerticalBorders: table.enableVerticalBorders, initialTop: {
                    to: scrollOffset ?? 0,
                    type: 'offset',
                }, itemType: LibraryItem.FOLDER, onColumnReordered: handleColumnReordered, onColumnResized: handleColumnResized, onScrollEnd: handleOnScrollEnd, overrideControls: overrideControls, size: table.size }, `folder-${serverId}-${currentFolderId}`));
        }
        default:
            return null;
    }
