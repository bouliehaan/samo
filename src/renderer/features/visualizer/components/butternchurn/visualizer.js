...
import { subscribeButterchurnPreset, useButterchurnSettings, usePlaybackType, useSettingsStore, useSettingsStoreActions, } from '/@/renderer/store';
...
export function getButterchurnPresetOptions(presets) {
    if (!presets)
        return [];
    return Object.fromEntries(Object.entries(presets).filter(([preset]) => !IGNORED_PRESETS.includes(preset)));
}
const VisualizerInner = () => {
    const { webAudio } = useWebAudio();
    const canvasRef = createRef();
    const containerRef = createRef();
    const visualizerRef = useRef(undefined);
    const isInitializedRef = useRef(false);
    const [isVisualizerReady, setIsVisualizerReady] = useState(false);
    const [librariesLoaded, setLibrariesLoaded] = useState(false);
    const butterchurnRef = useRef(null);
    const butterchurnPresetsRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const needsInitialization = context &&
            inputNodes.length > 0 &&
            canvas &&
            container &&
        ...
        async function initializeVisualizer(width, height) {
            const nodes = getVisualizerAudioNodes(webAudio, playbackType);
            if (!nodes.length || !canvas || !context || !librariesLoaded)
                return;
            canvas.width = width;
    ...
    useEffect(() => {
        ...
        if (presetNames.length > 0) {
            ...
            const presetName = currentPreset && presets[currentPreset] ? currentPreset : presetNames[0];
    ...
    useEffect(() => {
        ...
        if (butterchurnSettings.ignoredPresets && butterchurnSettings.ignoredPresets.length > 0) {
            presetList = presetList.filter((name) => !butterchurnSettings.ignoredPresets.includes(name));
        }
        ...
        const cycleToNextPreset = () => {
            ...
            if (butterchurnSettings.randomizeNextPreset) {
                ...
                const availablePresets = presetList.length > 1
                ...
                const nextIndex = currentIndex >= 0 && currentIndex < presetList.length - 1
                    ? currentIndex + 1
                    : 0;
    ...
    return (_jsxs("div", { className: styles.container, ref: containerRef, style: { opacity: isVisualizerReady ? opacity : 0 }, children: [_jsx("canvas", { className: styles.canvas, ref: canvasRef }), isVisualizerReady && _jsx(CurrentPresetDisplay, {})] }));
};
const CurrentPresetDisplay = () => {
    const currentPreset = useSettingsStore((store) => store.visualizer.butterchurn.currentPreset);
    return (_jsx(Text, { className: styles['preset-overlay'], isNoSelect: true, size: "sm", children: currentPreset }));
};
export const Visualizer = () => {
    ...
    useEffect(() => {
        ...
        const loadPresets = async () => {
            try {
                ...
                if (isMounted) {
                    butterchurnPresetsRef.current = getButterchurnPresetOptions(presetsModule.default);
    ...
    const handleNextPreset = () => {
        ...
        const currentPresetName = useSettingsStore.getState().visualizer.butterchurn.currentPreset;
        const currentIndex = currentPresetName ? presetList.indexOf(currentPresetName) : -1;
        const nextIndex = currentIndex >= 0 && currentIndex < presetList.length - 1 ? currentIndex + 1 : 0;
    ...
    return (_jsxs("div", { className: styles.container, children: [_jsxs(Group, { className: styles.iconGroup, gap: "xs", pos: "absolute", right: "var(--theme-spacing-sm)", top: "var(--theme-spacing-sm)", children: [_jsx(ActionIcon, { icon: "expand", iconProps: { size: 'lg' }, onClick: handleToggleFullscreen, variant: "subtle" }), _jsx(ActionIcon, { icon: "settings2", iconProps: { size: 'lg' }, onClick: openVisualizerSettingsModal, variant: "subtle" })] }), _jsxs(Group, { className: styles.iconGroup, gap: "xs", pos: "absolute", right: "var(--theme-spacing-sm)", style: { bottom: 'var(--theme-spacing-sm)' }, children: [_jsx(ActionIcon, { icon: "arrowLeftS", iconProps: { size: 'lg' }, onClick: handlePreviousPreset, variant: "subtle" }), _jsx(ActionIcon, { icon: "arrowRightS", iconProps: { size: 'lg' }, onClick: handleNextPreset, variant: "subtle" })] }), _jsx(ComponentErrorBoundary, { children: _jsx(VisualizerInner, {}) })] }));
};
