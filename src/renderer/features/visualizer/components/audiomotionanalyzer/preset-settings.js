...
export const PresetSettings = () => {
    ...
    const handleSavePreset = () => {
        ...
        const existingPreset = visualizer.audiomotionanalyzer.presets.find((p) => p.name === newPresetName.trim());
        if (existingPreset) {
            // Update existing preset
            const updatedPresets = visualizer.audiomotionanalyzer.presets.map((p) => p.id === existingPreset.id
                ? {
                    ...p,
    ...
    const handleUpdatePreset = () => {
        if (!selectedPreset || !newPresetName.trim())
            return;
        const selectedPresetObj = visualizer.audiomotionanalyzer.presets.find((p) => p.id === selectedPreset);
        ...
        const updatedPresets = visualizer.audiomotionanalyzer.presets.map((p) => p.id === selectedPreset
            ? {
                ...p,
                name: trimmedName,
                value: getCurrentSettingsAsPresetValue(),
    ...
    const handleDeletePreset = () => {
        if (!selectedPreset)
            return;
        const updatedPresets = visualizer.audiomotionanalyzer.presets.filter((p) => p.id !== selectedPreset);
        setSettings({
            visualizer: {
                audiomotionanalyzer: {
                    presets: updatedPresets,
    ...
    const handlePasteConfiguration = () => {
        ...
        try {
            ...
            const isUsingCustomGradient = gradientNamesToCheck.some((gradientName) => gradientName && pastedCustomGradientNames.includes(gradientName));
    ...
    return (_jsx(Fieldset, { legend: t('visualizer.presets'), children: _jsxs(Stack, { children: [_jsx(VisualizerSelect, { data: presetOptions, label: t('visualizer.selectPreset'), onChange: handlePresetChange, value: selectedPreset || undefined }), isSaving ? (_jsxs(Group, { grow: true, children: [_jsx(TextInput, { autoFocus: true, label: t('visualizer.presetName'), onChange: (e) => setNewPresetName(e.currentTarget.value), onKeyDown: (e) => {
                                if (e.key === 'Enter') {
                                    handleSavePreset();
                                }
                            ...
                            }, placeholder: t('visualizer.presetNamePlaceholder'), value: newPresetName }), _jsxs(Group, { style: { alignSelf: 'flex-end' }, children: [_jsx(Button, { onClick: () => setIsSaving(false), variant: "subtle", children: t('common.cancel', { postProcess: 'titleCase' }) }), _jsx(Button, { disabled: !newPresetName.trim(), onClick: handleSavePreset, variant: "filled", children: t('common.save', { postProcess: 'titleCase' }) })] })] })) : isRenaming ? (_jsxs(Group, { grow: true, children: [_jsx(TextInput, { autoFocus: true, label: t('visualizer.presetName'), onChange: (e) => setNewPresetName(e.currentTarget.value), onKeyDown: (e) => {
                                if (e.key === 'Enter') {
                                    handleUpdatePreset();
                                }
                                else if (e.key === 'Escape') {
                            ...
                            }, placeholder: t('visualizer.presetNamePlaceholder'), value: newPresetName }), _jsxs(Group, { style: { alignSelf: 'flex-end' }, children: [_jsx(Button, { onClick: () => setIsRenaming(false), variant: "subtle", children: t('common.cancel', { postProcess: 'titleCase' }) }), _jsx(Button, { disabled: !newPresetName.trim(), onClick: handleUpdatePreset, variant: "filled", children: t('common.save', { postProcess: 'titleCase' }) })] })] })) : isPasting ? (_jsxs(Stack, { children: [_jsx(Textarea, { autosize: true, label: t('visualizer.pasteConfiguration'), maxRows: 10, minRows: 5, onChange: (e) => setPasteValue(e.currentTarget.value), placeholder: t('visualizer.pasteConfigurationPlaceholder'), spellCheck: false, value: pasteValue }), _jsxs(Group, { children: [_jsx(Button, { onClick: handlePasteFromClipboard, variant: "subtle", children: t('visualizer.pasteFromClipboard') }), _jsx(Button, { onClick: () => setIsPasting(false), variant: "subtle", children: t('common.cancel', { postProcess: 'titleCase' }) }), _jsx(Button, { disabled: !pasteValue.trim(), onClick: handlePasteConfiguration, variant: "filled", children: t('visualizer.applyConfiguration') })] })] })) : (_jsxs(Group, { children: [_jsx(Button, { onClick: () => setIsSaving(true), variant: "default", children: t('visualizer.saveAsPreset') }), selectedPreset && (_jsxs(_Fragment, { children: [_jsx(Button, { onClick: () => {
                                        const preset = visualizer.audiomotionanalyzer.presets.find((p) => p.id === selectedPreset);
                                        if (preset) {
                                            setNewPresetName(preset.name);
                                            setIsRenaming(true);
                                        }
                                    }, variant: "default", children: t('visualizer.updatePreset') }), _jsx(Button, { onClick: handleDeletePreset, variant: "subtle", children: t('common.delete', { postProcess: 'titleCase' }) })] })), _jsx(Button, { onClick: handleCopyConfiguration, variant: "default", children: t('visualizer.copyConfiguration') }), _jsx(Button, { onClick: () => setIsPasting(true), variant: "default", children: t('visualizer.pasteConfiguration') })] }))] }) }));
};
