...
export const GetInfoAction = ({ disabled, items }) => {
    ...
    const onSelect = useCallback(async () => {
        ...
        if (filteredItems.length === 0)
            return;
        openModal({
            children: _jsx(ItemDetailsModal, { items: filteredItems }),
            size: 'lg',
            styles: {
                body: { paddingBottom: 'var(--theme-spacing-xl)' },
            },
            title: filteredItems.length === 1
                ? filteredItems[0]?.name ||
                    t('page.contextMenu.showDetails', { postProcess: 'sentenceCase' })
                : t('page.contextMenu.showDetails', { postProcess: 'sentenceCase' }),
        });
    }, [items, server, t]);
    return (_jsx(ContextMenu.Item, { disabled: disabled, leftIcon: "info", onSelect: onSelect, children: t('page.contextMenu.showDetails', { postProcess: 'sentenceCase' }) }));
};
