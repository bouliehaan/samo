...
import { isShuffleEnabled, mapShuffledToQueueIndex, subscribeCurrentTrack, subscribePlayerQueue, useFollowCurrentSong, useListSettings, usePlayerActions, usePlayerSong, usePlayerStore, } from '/@/renderer/store';
...
export const PlayQueue = forwardRef(({ enableScrollShadow = true, listKey, searchTerm, tableSize }, ref) => {
    ...
    useHotkeys([
        [
            ...
            () => {
                if (focused) {
                    const selectedItems = tableRef.current?.internalState.getSelected();
    ...
    return (_jsxs("div", { className: styles.container, ref: containerFocusRef, children: [_jsx(LoadingOverlay, { pos: "absolute", visible: isFetching }), _jsx(ItemTableList, { activeRowId: currentSongUniqueId, autoFitColumns: table.autoFitColumns, CellComponent: ItemTableListColumn, columns: table.columns, data: filteredData, enableAlternateRowColors: table.enableAlternateRowColors, enableDrag: true, enableExpansion: false, enableHeader: table.enableHeader, enableHorizontalBorders: table.enableHorizontalBorders, enableRowHoverHighlight: table.enableRowHoverHighlight, enableScrollShadow: enableScrollShadow, enableSelection: true, enableSelectionDialog: false, enableVerticalBorders: table.enableVerticalBorders, getRowId: "_uniqueId", groups: groups.length > 0 ? groups : undefined, initialTop: {
                    to: 0,
                    type: 'offset',
                }, itemType: LibraryItem.QUEUE_SONG, onColumnReordered: handleColumnReordered, onColumnResized: handleColumnResized, ref: mergedRef, size: tableSize ?? table?.size }), isEmpty && _jsx(EmptyQueueDropZone, {})] }));
});
const EmptyQueueDropZone = () => {
                ...
                if (args.self.type === DragTarget.QUEUE_SONG) {
                    ...
                    switch (args.source.type) {
                        case DragTarget.ALBUM: {
                            if (sourceServerId) {
                                playerContext.addToQueueByFetch(sourceServerId, args.source.id, sourceItemType, Play.NOW);
                            }
                            break;
                        }
                        ...
                        case DragTarget.FOLDER: {
                            ...
                            const { folders, songs } = (items || []).reduce((acc, item) => {
                                ...
                                else if (item._itemType === LibraryItem.FOLDER) {
                            ...
                            if (folderIds.length > 0) {
                                playerContext.addToQueueByFetch(sourceServerId, folderIds, LibraryItem.FOLDER, Play.NOW);
                            }
    ...
    return (_jsx(Flex, { align: "center", className: clsx(styles.dropZone, {
            [styles.draggedOver]: isDraggedOver,
        }), direction: "column", gap: "md", justify: "center", ref: ref, w: "100%" }));
};
