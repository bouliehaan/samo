...
export const tokenize = (value) => value
    .split(' ')
    .map((token) => token.trim())
    .filter(Boolean);
const lengthPenalty = (haystackLength, needleLength) => {
    if (haystackLength <= 0)
        return 0;
...
const scoreField = (haystackRaw, needle, needleTokens) => {
    ...
    if (needleTokens.length > 1) {
        ...
        const matched = needleTokens.filter((token) => {
            for (const hayToken of haystackTokens) {
                if (hayToken === token)
                    return true;
                if (hayToken.startsWith(token))
                    return true;
            }
            return false;
        }).length;
        if (matched === needleTokens.length)
            return SCORE_TOKEN_ALL;
        if (matched > 0)
            return SCORE_TOKEN_SOME * (matched / needleTokens.length);
    }
    return 0;
};
/**
 ...
 */
export const scoreCandidate = (primary, secondaries = [], needle, needleTokens) => {
    if (!needle)
        return 0;
    ...
    for (const secondary of secondaries) {
        ...
        if (score > bestSecondary)
            bestSecondary = score;
    }
    return primaryScore + bestSecondary * SECONDARY_FIELD_WEIGHT;
};
export const buildNeedleContext = (raw) => {
