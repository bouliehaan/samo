...
import { buildNeedleContext, scoreCandidate, } from '/@/renderer/features/search/utils/relevance';
...
const getAbsTitle = (item) => item.media?.metadata?.title ?? item.name ?? '';
const getAbsAuthor = (item) => {
    const meta = item.media?.metadata;
    return meta?.author ?? meta?.authors?.map((author) => author.name).join(', ') ?? '';
};
const entityBumpFor = (kind, ctx) => {
    if (ctx.needle.length > SHORT_QUERY_THRESHOLD)
        return 0;
    return ENTITY_GROUPS.has(kind) ? ENTITY_TYPE_BUMP : 0;
};
const rankSongs = (songs, ctx) => songs
    .map((song) => ({
    kind: 'song',
    score: scoreCandidate(song.name, [song.artistName, song.albumArtistName, song.album], ctx.needle, ctx.tokens) + entityBumpFor('songs', ctx),
    song,
}))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score || a.song.name.localeCompare(b.song.name))
    .slice(0, RESULT_LIMIT_PER_GROUP);
const rankAlbums = (albums, ctx) => albums
    .map((album) => ({
    album,
    kind: 'album',
    score: scoreCandidate(album.name, [album.albumArtistName], ctx.needle, ctx.tokens) +
        entityBumpFor('albums', ctx),
}))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score || a.album.name.localeCompare(b.album.name))
    .slice(0, RESULT_LIMIT_PER_GROUP);
const rankArtists = (artists, ctx) => artists
    .map((artist) => ({
    artist,
    kind: 'artist',
    score: scoreCandidate(artist.name, [], ctx.needle, ctx.tokens) +
        entityBumpFor('artists', ctx),
}))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score || a.artist.name.localeCompare(b.artist.name))
    .slice(0, RESULT_LIMIT_PER_GROUP);
const rankPlaylists = (playlists, ctx) => playlists
    .filter((playlist) => playlist?.id)
    .map((playlist) => ({
    kind: 'playlist',
    playlist,
    score: scoreCandidate(playlist.name, [playlist.owner], ctx.needle, ctx.tokens) +
        entityBumpFor('playlists', ctx),
}))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score || a.playlist.name.localeCompare(b.playlist.name))
    .slice(0, RESULT_LIMIT_PER_GROUP);
const rankRadio = (stations, ctx) => stations
    .map((station) => ({
    kind: 'radio',
    ...
    .slice(0, RESULT_LIMIT_PER_GROUP);
const rankAudiobooks = (items, ctx) => items
    .filter((item) => item.mediaType !== 'podcast')
    .map((item) => ({
    item,
    kind: 'audiobook',
    score: scoreCandidate(getAbsTitle(item), [getAbsAuthor(item), item.media?.metadata?.subtitle], ctx.needle, ctx.tokens) + entityBumpFor('audiobooks', ctx),
}))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score || getAbsTitle(a.item).localeCompare(getAbsTitle(b.item)))
    .slice(0, RESULT_LIMIT_PER_GROUP);
const rankPodcastShows = (items, ctx) => items
    .filter((item) => item.mediaType === 'podcast')
    .map((item) => ({
    item,
    kind: 'podcastShow',
    score: scoreCandidate(getAbsTitle(item), [getAbsAuthor(item)], ctx.needle, ctx.tokens) +
        entityBumpFor('podcastShows', ctx),
}))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score || getAbsTitle(a.item).localeCompare(getAbsTitle(b.item)))
    .slice(0, RESULT_LIMIT_PER_GROUP);
const rankPodcastEpisodes = (items, ctx) => {
    const candidates = [];
    for (const item of items) {
        ...
        for (const episode of episodes) {
            const score = scoreCandidate(episode.title, [episode.subtitle, getAbsTitle(item)], ctx.needle, ctx.tokens);
            if (score > 0) {
...
const groupMaxScore = (entries) => {
    if (entries.length === 0)
        return -Infinity;
    return entries[0].score;
};
const buildGroupOrder = (results) => {
    const entries = [
...
const collectBestMatches = (results) => {
    const all = [
        ...results.songs,
        ...results.albums,
        ...results.artists,
...
const combineAudiobookshelfItemsQueries = (queries) => ({
 ...
 */
export const useUnifiedSearch = (rawQuery) => {
    ...
    const radioStationsQuery = useQuery({
        enabled: enabled && Boolean(musicServer?.id),
        gcTime: 1000 * 60 * 60,
        queryFn: ({ signal }) => api.controller.getInternetRadioStations({
    ...
    const sourceErrors = useMemo(() => ({
        abs: getErrorMessage(audiobookshelfLibrariesQuery.error) ??
    ...
    const totalCount = results.songs.length +
