...
export const applyRatingOptimisticUpdates = (queryClient, variables, rating) => {
    ...
    const createRatingUpdater = (item) => ({
        ...item,
        userRating: rating,
    });
                ...
                if (data) {
                    pendingUpdates.push({
                        ...
                        updater: (prev) => {
                            if (!prev)
                                return prev;
                            const updatedItems = updateItemInArray(prev.items, itemIdSet, (item) => createRatingUpdater(item));
                ...
                if (data) {
                    pendingUpdates.push({
                        ...
                        updater: (prev) => {
                            if (!prev)
                                return prev;
                            const updatedPages = updateItemsInPages(prev.pages.filter((p) => !!p), itemIdSet, (item) => createRatingUpdater(item));
            ...
            infiniteLoaderQueries.forEach(([queryKey, data]) => {
                if (data) {
                    pendingUpdates.push({
                        ...
                        updater: (prev) => {
                            if (prev && prev.data) {
                                const updatedData = updateItemInArray(prev.data, itemIdSet, (item) => createRatingUpdater(item));
                ...
                if (data) {
                    pendingUpdates.push({
                        ...
                        updater: (prev) => {
                            if (!prev || !prev.songs)
                                return prev;
                            const updatedSongs = updateItemInArray(prev.songs, itemIdSet, (item) => createRatingUpdater(item));
...
export const applyRatingOptimisticUpdatesDeferred = (queryClient, variables, rating) => {
    const previousQueries = [];
    const itemIdSet = new Set();
    if (Array.isArray(variables.query.id)) {
        variables.query.id.forEach((id) => {
    ...
    switch (variables.query.type) {
        case LibraryItem.ALBUM: {
            ...
            collectQueries([variables.apiClientProps.serverId, 'item-list-infinite-loader', LibraryItem.ALBUM], 'album-infinite-loader');
            break;
        }
        case LibraryItem.ALBUM_ARTIST: {
            ...
            queryClient.setQueryData(queryKey, (prev) => {
                ...
                switch (type) {
                    ...
                    case 'artist-infinite-list': {
                        const updatedPages = updateItemsInPages(prev.pages || [], itemIdSet, (item) => ({ ...item, userRating: rating }));
                        return updatedPages ? { ...prev, pages: updatedPages } : prev;
                    }
                    ...
                    case 'album-artist-info': {
                        ...
                        return {
                            ...prev,
                            similarArtists: prev.similarArtists.map((a) => itemIdSet.has(a.id) ? { ...a, userRating: rating } : a),
                        };
                    }
                    ...
                    case 'song-list':
                    case 'top-songs': {
                        const updatedItems = updateItemInArray(prev.items || [], itemIdSet, (item) => ({ ...item, userRating: rating }));
                        return updatedItems ? { ...prev, items: updatedItems } : prev;
                    }
                    default:
                        return prev;
...
export const restoreRatingQueryData = (queryClient, previousQueries) => {
