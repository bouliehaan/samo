...
import { AlbumArtistListSort, AlbumListSort, ArtistListSort, GenreListSort, LibraryItem, PlaylistListSort, RadioListSort, ServerType, SongListSort, SortOrder, } from '/@/shared/types/domain-types';
export const ListSortByDropdown = ({ defaultSortByValue, disabled, itemType, listKey, onChange, target, }) => {
    const server = useCurrentServer();
    const { setSortBy, sortBy } = useSortByFilter(defaultSortByValue, listKey);
    const sortByLabel = (itemType && FILTERS[itemType][server.type].find((f) => f.value === sortBy)?.name) || '—';
    const handleSortByChange = (sortBy) => {
        setSortBy(sortBy);
        onChange?.(sortBy);
    };
    return (_jsxs(DropdownMenu, { disabled: disabled, position: "bottom-start", children: [_jsx(DropdownMenu.Target, { children: target ? (target) : (_jsx(Button, { disabled: disabled, variant: "subtle", children: sortByLabel })) }), _jsx(DropdownMenu.Dropdown, { children: FILTERS[itemType][server.type].map((f) => (_jsx(DropdownMenu.Item, { isSelected: f.value === sortBy, onClick: () => handleSortByChange(f.value), value: f.value, children: f.name }, `filter-${f.name}`))) })] }));
};
export const ListSortByDropdownControlled = ({ disabled, filters, itemType, setSortBy, sortBy, target, }) => {
    const server = useCurrentServer();
    ...
    return (_jsxs(DropdownMenu, { disabled: disabled, position: "bottom-start", children: [_jsx(DropdownMenu.Target, { children: target ? (target) : (_jsx(Button, { disabled: disabled, variant: "subtle", children: sortByLabel })) }), _jsx(DropdownMenu.Dropdown, { children: availableFilters.map((f) => (_jsx(DropdownMenu.Item, { isSelected: f.value === sortBy, onClick: () => handleSortByChange(f.value), value: f.value, children: f.name }, `filter-${f.name}`))) })] }));
};
...
const PLAYLIST_SONG_LIST_FILTERS = {
