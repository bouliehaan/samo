...
export const useRadioStore = createWithEqualityFn((set, get) => ({
    actions: {
        pause: () => {
            set({ isPlaying: false });
            usePlayerStoreBase.getState().mediaPause();
        },
        play: (streamUrl, stationName, stationArt) => {
            ...
            set((state) => {
                ...
                const isSwitchingStation = streamUrlExplicit && streamUrl !== state.currentStreamUrl;
