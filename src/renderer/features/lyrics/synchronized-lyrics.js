...
import { useLyricsDisplaySettings, useLyricsSettings, usePlaybackType, usePlayerActions, } from '/@/renderer/store';
...
const findActiveIndex = (lines, timeMs) => {
    if (lines.length === 0)
        return -1;
...
const getCenteredScrollTop = (container, lyric) => lyric.offsetTop + lyric.clientHeight / 2 - container.clientHeight / 2 + SCROLL_CENTER_OFFSET_PX;
export const SynchronizedLyrics = ({ lyrics, offsetMs, settingsKey = 'default', style, translatedLyrics, }) => {
    ...
    const settings = {
        ...
        fontSize: displaySettings.fontSize && displaySettings.fontSize !== 0
        ...
        scaleNonActive: displaySettings.scaleNonActive && displaySettings.scaleNonActive !== 0
    ...
    useEffect(() => {
        ...
        const applyActive = (newIdx) => {
            ...
            if (newIdx >= 0) {
                const node = container.querySelector(`[data-lyric-index="${newIdx}"]`);
                if (node) {
    ...
    return (_jsx("div", { className: clsx(styles.container, 'synchronized-lyrics overlay-scrollbar'), onMouseEnter: showScrollbar, onMouseLeave: hideScrollbar, ref: containerRef, style: {
            '--lyric-opacity': settings.opacityNonActive,
            '--lyric-scale': settings.scaleNonActive,
            '--lyric-scale-origin': settings.alignment,
            gap: `${settings.gap}px`,
            ...style,
        }, children: lyrics.map(([time, text], idx) => (_jsx(LyricLine, { alignment: settings.alignment, className: "lyric-line synchronized", "data-lyric-index": idx, fontSize: settings.fontSize, onClick: () => handleSeek(time), text: text +
