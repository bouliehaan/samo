...
const fetchRemoteLyricsAuto = async (song) => {
    if (!useSettingsStore.getState().lyrics.fetch)
        return null;
    const result = await lyricsIpc?.getRemoteLyricsBySong(song);
    if (!result)
        return null;
...
export const fetchRemoteLyricsById = async (params) => {
    const result = await lyricsIpc?.getRemoteLyricsByRemoteId(params);
    if (!result)
        return null;
    return formatLyrics(result);
};
export const clearLyricsCacheForSong = async (song) => {
    await lyricsIpc?.clearCacheForSong(song);
};
export const lyricsQueries = {
    search: (args) => queryOptions({
        gcTime: 1000 * 60,
        queryFn: async () => {
            if (!lyricsIpc) {
                return {};
    ...
    songLyrics: (args, song) => queryOptions({
        gcTime: Infinity,
        queryFn: async ({ signal }) => {
            if (!song)
                return { local: null, remoteAuto: null };
    ...
    songLyricsByRemoteId: (args) => queryOptions({
        gcTime: Infinity,
        queryFn: async () => {
            const q = args.query;
            if (!q?.remoteSongId || !q?.remoteSource)
                return null;
            return fetchRemoteLyricsById({
                remoteSongId: q.remoteSongId,
                remoteSource: q.remoteSource,
                song: q.song,
            });
        },
