...
const getNormalizedReleaseTypes = (album) => {
...
export const groupAlbumsByReleaseType = (albums, routeId, groupingType = 'primary') => {
    if (groupingType === 'all') {
...
export const releaseTypeToEnumMap = {
...
export const getArtistAlbumsGrouped = (albums, routeId, groupingType, artistReleaseTypeItems, t) => {
    const albumsByReleaseType = groupAlbumsByReleaseType(albums, routeId, groupingType);
    const enabledReleaseTypeEnums = new Set(artistReleaseTypeItems.filter((item) => !item.disabled).map((item) => item.id));
    const priorityMap = new Map();
    artistReleaseTypeItems
        .filter((item) => !item.disabled)
        .forEach((item, index) => {
        const releaseTypeKey = Object.keys(releaseTypeToEnumMap).find((key) => releaseTypeToEnumMap[key] === item.id);
        if (releaseTypeKey) {
            priorityMap.set(releaseTypeKey, index);
        }
    });
    const getDisplayNameForType = (releaseType) => {
        switch (releaseType) {
            case 'album':
    ...
    const getSecondaryTypePriorityKey = (releaseType) => {
        if (releaseType.includes('/')) {
            const types = releaseType.split('/');
    ...
    const isReleaseTypeEnabled = (releaseType) => {
        if (releaseType.includes('/')) {
            const types = releaseType.split('/');
...
export const useArtistAlbumsGrouped = (albums, routeId) => {
