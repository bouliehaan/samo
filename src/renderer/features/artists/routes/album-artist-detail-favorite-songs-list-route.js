...
const AlbumArtistDetailFavoriteSongsListRoute = () => {
    ...
    if (!tableConfig || columns.length === 0) {
        return (_jsx(AnimatedPage, { children: _jsx(ListContext.Provider, { value: providerValue, children: _jsx(AlbumArtistDetailFavoriteSongsListHeader, { data: sortedSongs, itemCount: itemCount, title: detailQuery?.data?.name || 'Unknown' }) }) }));
    }
    return (_jsx(AnimatedPage, { children: _jsxs(ListContext.Provider, { value: providerValue, children: [_jsx(AlbumArtistDetailFavoriteSongsListHeader, { data: sortedSongs, itemCount: itemCount, title: detailQuery?.data?.name || 'Unknown' }), _jsx(FilterBar, { children: _jsx(AlbumArtistDetailFavoriteSongsListHeaderFilters, {}) }), _jsx(ItemTableList, { activeRowId: currentSongId, autoFitColumns: tableConfig.autoFitColumns, CellComponent: ItemTableListColumn, columns: columns, data: sortedSongs, enableAlternateRowColors: tableConfig.enableAlternateRowColors, enableDrag: true, enableExpansion: false, enableHeader: tableConfig.enableHeader, enableHorizontalBorders: tableConfig.enableHorizontalBorders, enableRowHoverHighlight: tableConfig.enableRowHoverHighlight, enableSelection: true, enableSelectionDialog: false, enableVerticalBorders: tableConfig.enableVerticalBorders, itemType: LibraryItem.SONG, onColumnReordered: handleColumnReordered, onColumnResized: handleColumnResized, overrideControls: overrideControls, size: tableConfig.size })] }) }));
};
const AlbumArtistDetailFavoriteSongsListRouteWithBoundary = () => {
    return (_jsx(PageErrorBoundary, { children: _jsx(AlbumArtistDetailFavoriteSongsListRoute, {}) }));
};
