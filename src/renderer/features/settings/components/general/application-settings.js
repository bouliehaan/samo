...
export const ApplicationSettings = memo(() => {
    ...
    const options = [
        {
            control: (_jsx(Select, { data: languages.map((language) => ({
                    label: `${language.label} (${language.value})`,
                    value: language.value,
                })), onChange: handleChangeLanguage, value: settings.language })),
        ...
        {
            control: (_jsx(Switch, { "aria-label": t('setting.offlineMode', { postProcess: 'sentenceCase' }), checked: settings.offlineMode, onChange: (e) => {
    ...
    return _jsx(SettingsSection, { options: options });
});
