...
const getAudioDevices = async () => {
    ...
    return (devices || []).filter((dev) => dev.kind === 'audiooutput');
};
export const useAudioDevices = (playbackType = PlayerType.WEB) => {
    ...
    useEffect(() => {
        ...
        getAudioDevices()
            .then((dev) => {
            const uniqueDevices = dev.filter((d, index, self) => index === self.findIndex((t) => t.deviceId === d.deviceId));
...
export const AudioSettings = memo(() => {
    ...
    const playerTypeSelect = (_jsx(Select, { data: [
            {
        ...
        ], defaultValue: settings.type, disabled: !isElectron() || isPlaying, onChange: (value) => {
            if (!value) {
                return;
            }
            setSettings({
    ...
    const audioOptions = [
        {
            control: isPlaying ? (_jsx(Tooltip, { label: t('player.pausePlaybackToChangeSetting', {
                    postProcess: 'titleCase',
                }), children: _jsx("div", { children: playerTypeSelect }) })) : (playerTypeSelect),
        ...
        {
            control: (_jsx(Select, { clearable: true, data: audioDevices, defaultValue: settings.type === PlayerType.LOCAL
                    ? settings.mpvAudioDeviceId
                    : settings.audioDeviceId, disabled: !isElectron() || isPlaying, onChange: (e) => setSettings({
                    playback: settings.type === PlayerType.LOCAL
                        ? { mpvAudioDeviceId: e }
                        : { audioDeviceId: e },
                }) })),
        ...
        {
            control: (_jsx(Switch, { defaultChecked: settings.preservePitch, onChange: (e) => {
                    setSettings({
        ...
        {
            control: (_jsx(Switch, { defaultChecked: settings.audioFadeOnStatusChange, onChange: (e) => {
                    setSettings({
                        playback: {
    ...
    return _jsx(SettingsSection, { options: audioOptions });
});
