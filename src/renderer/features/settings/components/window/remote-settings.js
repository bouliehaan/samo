...
export const RemoteSettings = memo(() => {
    ...
    const debouncedEnableRemote = debounce(async (enabled) => {
    ...
    const debouncedChangeRemotePort = debounce(async (port) => {
    ...
    const controlOptions = [
        {
            control: (_jsx(Switch, { defaultChecked: settings.enabled, onChange: async (e) => {
                    const enabled = e.currentTarget.checked;
                    await debouncedEnableRemote(enabled);
                } })),
            description: (_jsxs(Text, { isMuted: true, isNoSelect: true, size: "sm", children: [t('setting.enableRemote', {
                        context: 'description',
                        postProcess: 'sentenceCase',
                    }), ' ', _jsx("a", { href: url, rel: "noreferrer noopener", target: "_blank", children: url })] })),
            isHidden,
            title: t('setting.enableRemote', { postProcess: 'sentenceCase' }),
        },
        {
            control: (_jsx(NumberInput, { max: 65535, onBlur: async (e) => {
                    if (!e)
                        return;
        ...
        {
            control: (_jsx(TextInput, { defaultValue: settings.username, onBlur: (e) => {
        ...
        {
            control: (_jsx(TextInput, { defaultValue: settings.password, onBlur: (e) => {
    ...
    return (_jsx(SettingsSection, { options: controlOptions, title: t('page.setting.remote', { postProcess: 'sentenceCase' }) }));
});
