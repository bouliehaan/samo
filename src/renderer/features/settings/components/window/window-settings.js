...
export const WindowSettings = memo(() => {
    ...
    const windowOptions = [
        {
            control: (_jsx(Switch, { "aria-label": "toggle hiding tray", defaultChecked: settings.tray, disabled: !isElectron(), onChange: (e) => {
                    if (!e)
                        return;
        ...
        {
            control: (_jsx(Switch, { "aria-label": "Toggle minimize to tray", defaultChecked: settings.tray, disabled: !isElectron(), onChange: (e) => {
                    if (!e)
                        return;
        ...
        {
            control: (_jsx(Switch, { "aria-label": "Toggle exit to tray", defaultChecked: settings.exitToTray, disabled: !isElectron(), onChange: (e) => {
                    if (!e)
                        return;
        ...
        {
            control: (_jsx(Switch, { "aria-label": "Toggle start in tray", defaultChecked: settings.startMinimized, disabled: !isElectron(), onChange: (e) => {
                    if (!e)
                        return;
        ...
        {
            control: (_jsx(Switch, { "aria-label": "Toggle prevent sleep on playback", defaultChecked: settings.preventSleepOnPlayback, disabled: !isElectron(), onChange: (e) => {
                    if (!e)
                        return;
                    localSettings?.set('window_prevent_sleep_on_playback', e.currentTarget.checked);
                    setSettings({
                        window: {
    ...
    return (_jsx(SettingsSection, { options: windowOptions, title: t('page.setting.application', { postProcess: 'sentenceCase' }) }));
});
