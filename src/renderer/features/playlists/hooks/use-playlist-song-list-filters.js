...
import { parseArrayParam, parseBooleanParam, parseCustomFiltersParam, parseIntParam, setMultipleSearchParams, setSearchParam, } from '/@/renderer/utils/query-params';
...
export const usePlaylistSongListFilters = () => {
    ...
    const albumArtistIds = useMemo(() => parseArrayParam(searchParams, FILTER_KEYS.SONG.ALBUM_ARTIST_IDS), [searchParams]);
    const genreId = useMemo(() => parseArrayParam(searchParams, FILTER_KEYS.SONG.GENRE_ID), [searchParams]);
    const artistIds = useMemo(() => parseArrayParam(searchParams, FILTER_KEYS.SONG.ARTIST_IDS), [searchParams]);
    const minYear = useMemo(() => parseIntParam(searchParams, FILTER_KEYS.SONG.MIN_YEAR), [searchParams]);
    const maxYear = useMemo(() => parseIntParam(searchParams, FILTER_KEYS.SONG.MAX_YEAR), [searchParams]);
    const favorite = useMemo(() => parseBooleanParam(searchParams, FILTER_KEYS.SONG.FAVORITE), [searchParams]);
    const hasRating = useMemo(() => parseBooleanParam(searchParams, FILTER_KEYS.SONG.HAS_RATING), [searchParams]);
    const custom = useMemo(() => parseCustomFiltersParam(searchParams, FILTER_KEYS.SONG._CUSTOM), [searchParams]);
    const setAlbumArtistIds = useCallback((value) => {
        runInUrlTransition(() => {
            setSearchParams((prev) => setSearchParam(prev, FILTER_KEYS.SONG.ALBUM_ARTIST_IDS, value), { replace: true });
        });
    }, [setSearchParams]);
    const setGenreId = useCallback((value) => {
        runInUrlTransition(() => {
    ...
    const setArtistIds = useCallback((value) => {
        runInUrlTransition(() => {
    ...
    const setMinYear = useCallback((value) => {
        runInUrlTransition(() => {
    ...
    const setMaxYear = useCallback((value) => {
        runInUrlTransition(() => {
    ...
    const setFavorite = useCallback((value) => {
        runInUrlTransition(() => {
    ...
    const setHasRating = useCallback((value) => {
        runInUrlTransition(() => {
    ...
    const setAlbumArtistIdsMode = useCallback((value) => setAlbumArtistIdsModeStore(value), [setAlbumArtistIdsModeStore]);
    const setArtistIdsMode = useCallback((value) => setArtistIdsModeStore(value), [setArtistIdsModeStore]);
    const setGenreIdsMode = useCallback((value) => setGenreIdsModeStore(value), [setGenreIdsModeStore]);
    const setCustom = useCallback((value) => {
        runInUrlTransition(() => {
            setSearchParams((prev) => {
                ...
                const filteredNewCustom = Object.fromEntries(Object.entries(newCustom).filter(([, value]) => value !== null && value !== undefined));
    ...
    const clear = useCallback(() => {
        runInUrlTransition(() => {
            setSearchParams((prev) => setMultipleSearchParams(prev, {
            ...
            }, new Set([FILTER_KEYS.SONG._CUSTOM])), { replace: true });
        });
    }, [setSearchParams]);
    const query = useMemo(() => ({
