...
export function applyClientSideSongFilters(songs, query) {
    let result = songs;
    const favorite = query[FILTER_KEYS.SONG.FAVORITE];
    if (favorite === true) {
    ...
    const hasRating = query[FILTER_KEYS.SONG.HAS_RATING];
    if (hasRating === true) {
    ...
    const albumArtistIds = query[FILTER_KEYS.SONG.ALBUM_ARTIST_IDS];
    if (albumArtistIds?.length) {
        if (albumArtistIdsMode === 'and') {
            result = result.filter((s) => albumArtistIds.every((id) => s.albumArtists?.some((a) => a.id === id)));
        }
        else {
    ...
    const artistIds = query[FILTER_KEYS.SONG.ARTIST_IDS];
    if (artistIds?.length) {
        if (artistIdsMode === 'and') {
            result = result.filter((s) => artistIds.every((id) => s.artists?.some((a) => a.id === id)));
        }
        else {
    ...
    const genreIds = query[FILTER_KEYS.SONG.GENRE_ID];
    if (genreIds?.length) {
        if (genreIdsMode === 'and') {
            result = result.filter((s) => genreIds.every((id) => s.genres?.some((g) => g.id === id)));
        }
        else {
    ...
    const minYear = query[FILTER_KEYS.SONG.MIN_YEAR];
    if (minYear != null) {
        result = result.filter((s) => s.releaseYear != null && s.releaseYear >= minYear);
    }
    const maxYear = query[FILTER_KEYS.SONG.MAX_YEAR];
    if (maxYear != null) {
        result = result.filter((s) => s.releaseYear != null && s.releaseYear <= maxYear);
    }
    return result;
}
export function usePlaylistTrackList(data) {
