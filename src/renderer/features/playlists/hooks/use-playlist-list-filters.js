...
export const usePlaylistListFilters = () => {
    const sortByFilter = useSortByFilter(null, ItemListKey.PLAYLIST);
    const sortOrderFilter = useSortOrderFilter(null, ItemListKey.PLAYLIST);
    const { searchTerm, setSearchTerm } = useSearchTermFilter('');
    const [searchParams, setSearchParams] = useSearchParams();
    const custom = useMemo(() => parseCustomFiltersParam(searchParams, FILTER_KEYS.PLAYLIST.CUSTOM), [searchParams]);
