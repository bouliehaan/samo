...
import { getCurrentChapterIndex, useAudiobookChapters, useAudiobookDuration, useAudiobookItem, useAudiobookPosition, useAudiobookServer, useAudiobookStore, } from '/@/renderer/store/audiobook.store';
import { usePlaybackOwnerStore, usePlaybackSource, } from '/@/renderer/store/playback-owner.store';
import { usePodcastEpisode, usePodcastItem, usePodcastServer, usePodcastStore, } from '/@/renderer/store/podcast.store';
...
export function useNowPlaying() {
    ...
    const { currentStationArt, currentStreamUrl: radioStreamUrl, isPlaying: isRadioPlaying, metadata: radioMetadata, stationName, } = useRadioPlayer();
    ...
    if (source === 'audiobook' && audiobookItem) {
        const chapterIndex = getCurrentChapterIndex(audiobookChapters, audiobookPosition, audiobookDuration);
        const currentChapterTitle = chapterIndex >= 0 ? audiobookChapters[chapterIndex].title || '' : '';
        return {
            artist: audiobookAuthor(audiobookItem),
            artwork: audiobookItem && audiobookServer
...
function audiobookArtworkUrl(server, item) {
    if (!item.id || !server.url || !server.credential)
        return undefined;
    const base = server.url.replace(/\/+$/, '');
    return `${base}/api/items/${item.id}/cover?token=${encodeURIComponent(server.credential)}`;
}
function audiobookAuthor(item) {
...
function podcastShowTitle(item) {
    return item.media?.metadata?.title || item.name || 'Podcast';
}
function radioArtworkUrl(stationArt) {
    if (!stationArt)
        return undefined;
