...
import { useAppStore, useAppStoreActions, useCommandPalette, usePlayerHydrated, usePlayerPlaybackControlsState, } from '/@/renderer/store';
...
export const useNativeMenuSync = () => {
    ...
    const { repeat: playerRepeat, shuffle: playerShuffle, status: playerStatus } = usePlayerPlaybackControlsState();
    useEffect(() => {
        if (!isElectron()) {
            return undefined;
        }
    ...
    useEffect(() => {
        ...
        window.api.utils.rendererOpenReleaseNotes(() => {
            openReleaseNotesModal(t('common.newVersion', {
