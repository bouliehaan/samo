...
import { getServerById, useAuthHydrated, useAuthStoreActions, useCurrentServerId, } from '/@/renderer/store';
...
const isNetworkError = (error) => {
    const message = error.message && typeof error.message === 'string' ? error.message : null;
...
export const useServerAuthenticated = () => {
    ...
    const priorAuthSignature = useRef(undefined);
    ...
    const refreshServerInfo = useCallback(async (serverWithAuth) => {
        try {
    ...
    const authenticateServer = useCallback(async (serverWithAuth, retryAttempt = 0) => {
        try {
            ...
            try {
                ...
                const isForbiddenError = getUserInfoError?.response?.status === 401 ||
