...
export const useTablePaneSync = ({ enableDrag, enableDragScroll, enableHeader, handleRef, onScrollEndRef, pinnedLeftColumnCount, pinnedLeftColumnRef, pinnedRightColumnCount, pinnedRightColumnRef, pinnedRowRef, rowRef, scrollContainerRef, scrollShadowStore, }) => {
    ...
    useEffect(() => {
        const header = pinnedRowRef.current?.childNodes[0];
        const row = rowRef.current?.childNodes[0];
        const pinnedLeft = pinnedLeftColumnRef.current?.childNodes[0];
        const pinnedRight = pinnedRightColumnRef.current?.childNodes[0];
        if (!row)
            return;
        ...
        const setActiveElement = (e) => {
            activeElement.element = e.currentTarget;
        };
        const setActiveElementFromWheel = (e) => {
            activeElement.element = e.currentTarget;
        };
        const markElementAsScrolling = (element) => {
            ...
            const timeout = setTimeout(() => {
                ...
                if (scrollElement && onScrollEndRef.current) {
                    onScrollEndRef.current(scrollElement.scrollTop, (handleRef.current?.internalState ??
                        undefined));
                }
        ...
        const syncScroll = (e) => {
            ...
            const shouldSync = currentElement === activeElement.element || scrollingElements.has(currentElement);
            if (!shouldSync)
                return;
        ...
        let heightSyncDebounceTimeout = null;
