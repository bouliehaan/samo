...
export const useStickyTableGroupRows = ({ containerRef, enabled, getRowHeight, groups, headerHeight, mainGridRef, shouldShowStickyHeader, stickyHeaderTop, }) => {
    const { windowBarStyle } = useWindowSettings();
    const [stickyGroupIndex, setStickyGroupIndex] = useState(null);
    ...
    useEffect(() => {
        ...
        const mainGrid = mainGridContainer.childNodes[0];
        if (!mainGrid) {
            return;
        }
