...
export const useTableColumnModel = ({ autoFitColumns, centerContainerWidth, columns, totalContainerWidth, }) => {
    ...
    const calculatedColumnWidths = useMemo(() => {
        ...
        if (autoFitColumns) {
            const pinnedWidth = parsedColumns.reduce((sum, col, idx) => (col.pinned !== null ? sum + baseWidths[idx] : sum), 0);
            const unpinnedIndices = [];
            parsedColumns.forEach((col, idx) => {
                if (col.pinned === null) {
                    unpinnedIndices.push(idx);
                }
            });
            const unpinnedReferenceWidth = unpinnedIndices.reduce((sum, idx) => sum + baseWidths[idx], 0);
    ...
    const pinnedLeftColumnCount = useMemo(() => parsedColumns.filter((col) => col.pinned === 'left').length, [parsedColumns]);
    const pinnedRightColumnCount = useMemo(() => parsedColumns.filter((col) => col.pinned === 'right').length, [parsedColumns]);
