...
import { recordRecentAlbum, recordRecentArtist, recordRecentPlaylist, recordRecentSong, useAppStore, } from '/@/renderer/store';
...
export const useDefaultItemListControls = (args) => {
    ...
    const controls = useMemo(() => {
        return {
            onClick: ({ event, internalState, item }) => {
                ...
                if (event.ctrlKey || event.metaKey) {
                    ...
                    if (isCurrentlySelected) {
                        ...
                        const filteredSelected = currentSelected.filter((selectedItem) => typeof selectedItem === 'object' &&
                            selectedItem !== null &&
                ...
                // Check if shift key is held for range selection
                    ...
                    const selectedItems = internalState.getSelected();
                    const isOnlySelected = selectedItems.length === 1 &&
            ...
            onColumnResized: onColumnResized
                ? ({ columnId, width }) => onColumnResized(columnId, width)
                : undefined,
            onDoubleClick: ({ internalState, item, itemType, meta }) => {
            ...
            onExpand: ({ item, itemType }) => {
                ...
                if (globalExpanded?.item?.id === item.id) {
                    ...
                    const itemForStore = {
                        ...itemListItem,
                        imageId: itemListItem.imageId ?? null,
                    };
                    setGlobalExpanded({
                        item: itemForStore,
                        itemType,
            ...
            onFavorite: ({ favorite, item, itemType, }) => {
                if (!item) {
                    return;
                }
            ...
            onMore: ({ event, internalState, item, itemType }) => {
                if (!item || !event) {
                    return;
                }
                ...
                const actualItemType = itemType === LibraryItem.PLAYLIST_SONG || itemType === LibraryItem.QUEUE_SONG
                    ? itemType
                    : item?._itemType || itemTypeMapping[itemType] || itemType;
                // If no internalState, call ContextMenuController directly
                if (!internalState) {
                    return ContextMenuController.call({
                        cmd: { items: [item], type: actualItemType },
                        event,
                    });
                }
                const rowId = internalState.extractRowId(item);
                ...
                const selectedItemType = itemType === LibraryItem.PLAYLIST_SONG || itemType === LibraryItem.QUEUE_SONG
                    ? itemType
                    : selectedItems.length > 0 && selectedItems[0]?._itemType
                        ? selectedItems[0]._itemType
                        : itemTypeMapping[itemType] || itemType;
                return ContextMenuController.call({
                    cmd: { items: selectedItems, type: selectedItemType },
                    event,
                });
            },
