...
export const useItemListColumnReorder = ({ itemListKey, tableKey = 'main', }) => {
    const { setList } = useSettingsStoreActions();
    const handleColumnReordered = useCallback((columnIdFrom, columnIdTo, edge) => {
        ...
        const updatedMovedColumn = targetColumn.pinned === 'left'
            ? { ...movedColumn, pinned: 'left' }
            : targetColumn.pinned === 'right'
                ? { ...movedColumn, pinned: 'right' }
        ...
        if (tableKey === 'detail') {
            setList(itemListKey, { detail: { columns: newColumns } });
        }
        else {
            setList(itemListKey, {
                table: {
