...
import { getDataRows } from '/@/renderer/components/item-card/item-card';
import { useSettingsStore } from '/@/renderer/store';
import { LibraryItem } from '/@/shared/types/domain-types';
import { TableColumn } from '/@/shared/types/types';
const getDefaultRowsForItemType = (itemType, type) => {
    const allRows = getDataRows(type);
    const rowMap = new Map(allRows.map((row) => [row.id, row]));
    switch (itemType) {
        case LibraryItem.ALBUM:
            return [rowMap.get('name'), rowMap.get('albumArtists')].filter((row) => row !== undefined);
        case LibraryItem.ALBUM_ARTIST:
            return [rowMap.get('name')].filter((row) => row !== undefined);
...
const getRowIdFromTableColumn = (tableColumn) => {
    const columnToRowIdMap = {
...
export const useGridRows = (itemType, listKey, size) => {
    const gridRowsConfig = useSettingsStore((state) => listKey ? state.lists[listKey]?.grid?.rows : undefined);
    const type = size === 'compact' ? 'compact' : 'poster';
    return useMemo(() => {
        const allRows = getDataRows(type);
