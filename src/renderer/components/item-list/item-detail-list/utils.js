...
const FIXED_TRACK_COLUMN_WIDTHS = {
...
const HOVER_ONLY_COLUMNS = [
    TableColumn.ACTIONS,
    TableColumn.USER_FAVORITE,
    TableColumn.USER_RATING,
];
const NO_HORIZONTAL_PADDING_COLUMNS = [
    TableColumn.ACTIONS,
    TableColumn.USER_FAVORITE,
    TableColumn.USER_RATING,
];
export function getTrackColumnFixed(columnId) {
    const width = FIXED_TRACK_COLUMN_WIDTHS[columnId];
    return width !== undefined
        ? { fixedWidth: width, isFixedColumn: true }
        : { fixedWidth: 0, isFixedColumn: false };
}
export function isNoHorizontalPaddingColumn(columnId) {
    return NO_HORIZONTAL_PADDING_COLUMNS.includes(columnId);
}
export function isTrackColumnHoverOnly(columnId) {
    return HOVER_ONLY_COLUMNS.includes(columnId);
}
export function shouldShowHoverOnlyColumnContent(columnId, isRowHovered, song) {
