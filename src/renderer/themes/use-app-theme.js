...
import { useAccent, useFontSettings, useNativeAspectRatio, useThemeSettings, } from '/@/renderer/store/settings.store';
...
export const useAppTheme = (overrideTheme) => {
    ...
    const themeInlineStylesRef = useRef(null);
    const getCurrentTheme = () => window.matchMedia('(prefers-color-scheme: dark)').matches;
    const [isDarkTheme, setIsDarkTheme] = useState(getCurrentTheme());
    const { followSystemTheme, primaryShade, theme, themeDark, themeLight, useThemeAccentColor, useThemePrimaryShade, } = useThemeSettings();
    const mqListener = (e) => {
        setIsDarkTheme(e.matches);
    };
    const applyInlineStylesheets = useCallback((inlineCssStrings = []) => {
    ...
    useEffect(() => {
        if (type === FontType.SYSTEM && system) {
            ...
            root.style.setProperty('--theme-content-font-family', 'dynamic-font, "Noto Sans JP", "Noto Sans Hebrew", sans-serif');
            ...
            const root = document.documentElement;
            root.style.setProperty('--theme-content-font-family', `${builtIn}, "Noto Sans JP", "Noto Sans Hebrew", sans-serif`);
        }
    ...
    useEffect(() => {
        ...
        const shadeIndex = Math.min(9, Math.max(0, typeof effectivePrimaryShade === 'object'
