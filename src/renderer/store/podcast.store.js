...
import { createAbsPlaybackStore, } from '/@/renderer/store/abs-playback.store';
...
const resumeKey = (itemId, episodeId) => `${itemId}::${episodeId}`;
const rememberPodcastPlaybackSession = (server, item, episode, position) => {
...
const sortedEpisodes = (item) => (item.media?.episodes ?? [])
    .slice()
...
const { store: usePodcastStore, selectors } = createAbsPlaybackStore({
    ...
    getResumeKey: (state) => state.item && state.episode ? resumeKey(state.item.id, state.episode.id) : null,
    ...
    resolvePlaySession: async (_server, item, episode) => {
        ...
        const localResume = usePodcastStore.getState().resumeByEpisodeKey[resumeKey(libraryItem.id, ep.id)];
        const serverResume = session.currentTime ?? 0;
        const resumePosition = localResume !== undefined ? localResume : serverResume;
        const playSessionEpisode = session.libraryItem?.media?.episodes?.find((e) => e.id === ep.id);
        const duration = playSessionEpisode?.duration ??
            playSessionEpisode?.audioFile?.duration ??
            seedDuration;
        const sessionEpisodes = session.libraryItem?.media?.episodes;
        const itemToStore = sessionEpisodes && sessionEpisodes.length > 0 ? session.libraryItem : libraryItem;
        return {
            contentUrl,
            duration,
    ...
    source: 'podcast',
    updateResumeOnSeek: (state, position) => state.item && state.episode
