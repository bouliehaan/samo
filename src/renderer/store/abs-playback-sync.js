...
export function createAbsPlaybackSyncHandle(logLabel, getSlice) {
    ...
    const syncProgress = (options) => {
        ...
        const now = Date.now();
        const timeListened = options.countListeningTime && lastServerSyncAtMs > 0
...
export function wireAbsPlaybackOwnerHandoff(options) {
    usePlaybackOwnerStore.subscribe((state) => state.source, (source) => {
