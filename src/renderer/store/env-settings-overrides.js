...
export function getEnvSettingsOverrides() {
    const w = getWin();
    const get = (key) => {
        const v = w[key];
        if (typeof v !== 'string')
            return undefined;
        if (isUnsubstitutedPlaceholder(v))
            return undefined;
        return v;
    };
    const overrides = {};
...
function isUnsubstitutedPlaceholder(s) {
