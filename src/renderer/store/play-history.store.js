...
export const playHistoryKey = ({ itemId, serverId, type }) => `${type}:${serverId}:${itemId}`;
const getRecentItemKey = (entry) => playHistoryKey({
    itemId: entry.itemId,
    serverId: entry.serverId,
    type: entry.mediaType,
});
const compactItems = (items) => items
    .slice()
    .sort((a, b) => b.selectedAt - a.selectedAt)
    .slice(0, RECENT_ITEM_LIMIT);
export const usePlayHistoryStore = create()(persist((set) => ({
    actions: {
        clear: () => set({ items: [] }),
        pruneStale: ({ knownItemIds, mediaType, serverId }) => set((state) => ({
        ...
        record: (entry) => {
            if (!entry.itemId || !entry.serverId)
                return;
...
export const recordRecentItem = (entry) => {
...
export const recordRecentAlbum = (album) => {
    if (!album._serverId)
        return;
    recordRecentItem({
        artwork: {
...
export const recordRecentArtist = (artist, fallback) => {
    ...
    recordRecentItem({
        ...
        mediaType: 'artist',
        serverId,
        subtitle: withDetail('Artist', 'albumCount' in artist ? countText(artist.albumCount, 'album') : undefined),
        title: artist.name,
    });
};
export const recordRecentPlaylist = (playlist) => {
...
export const recordRecentSong = (song) => {
    if (!song._serverId)
        return;
    recordRecentItem({
        artwork: {
...
export const recordRecentRadioStation = (station, serverId) => {
    if (!serverId)
        return;
    recordRecentItem({
        artwork: {
...
export const recordRecentAudiobook = (item, serverId) => {
    if (!serverId)
        return;
...
export const recordRecentPodcast = (item, serverId) => {
    if (!serverId)
        return;
    recordRecentItem({
        artwork: {
