...
import { createInitialPlayerTransportSlice, } from '/@/renderer/store/player/slices';
import { createSelectors } from '/@/renderer/lib/zustand';
import { isStructuredMusicContext, rememberMusicPlaybackSession, SONG_CONTEXT, } from '/@/renderer/store/last-playback-session.store';
import { usePlaybackOwnerStore } from '/@/renderer/store/playback-owner.store';
import { useSettingsStore } from '/@/renderer/store/settings.store';
import { setTimestamp as setTimestampStore, useTimestampStoreBase, } from '/@/renderer/store/timestamp.store';
import { migratePlayerStorePersist, playerStoreStorage, setPlayerStoreHydratedForPersistence, } from '/@/renderer/store/utils';
import { addIndexesToShuffled, adjustShuffledIndexesForInsertion, calculateNextIndex, calculateNextSong, findShuffledPositionForQueueIndex, generateShuffledIndexes, isShuffleEnabled, mapShuffledToQueueIndex, regenerateShuffledIndexesIfNeeded, } from '/@/renderer/store/player-queue-math';
import { computePlayerData, getCurrentSongFromState, getPlayerDataFromState, getPlaybackInputs, getQueueFromState, getQueueOrderFromState, isFirstTrackInQueueFromState, isLastTrackInQueueFromState, playbackInputsEqual, touchQueueRevision, } from '/@/renderer/store/player-derived';
import { shuffleInPlace } from '/@/renderer/utils/shuffle';
export { calculateNextSong, isShuffleEnabled, mapShuffledToQueueIndex, } from '/@/renderer/store/player-queue-math';
import { Play, PlayerRepeat, PlayerShuffle, PlayerStatus, PlayerType, } from '/@/shared/types/types';
function emitPlayerPlayEvent(targetSongUniqueId, set, get) {
    // If playSongId is provided, find the song and start playback on it
    if (targetSongUniqueId) {
        let playIndex;
        set((state) => {
            const queue = getQueueFromState(state);
            const queueIndex = queue.items.findIndex((item) => item._uniqueId === targetSongUniqueId);
            if (queueIndex !== -1) {
                if (state.player.shuffle === PlayerShuffle.TRACK &&
                    state.queue.shuffled.length > 0) {
                    // Find the shuffled position for this queue index
                    const shuffledPosition = state.queue.shuffled.findIndex((idx) => idx === queueIndex);
...
export const usePlayerStoreBase = createWithEqualityFn()(persist(subscribeWithSelector(immer((set, get) => ({
        ...
        switch (playType) {
            case Play.LAST: {
                set((state) => {
                    ...
                    if (isShuffleEnabled(state)) {
                        ...
                        const newIndexes = Array.from({ length: newUniqueIds.length }, (_, i) => oldQueueLength + i);
            ...
            case Play.NEXT: {
                set((state) => {
                    ...
                    const insertPosition = state.player.shuffle === PlayerShuffle.TRACK
                    ...
                    if (isShuffleEnabled(state)) {
                        ...
                        const adjustedShuffled = adjustShuffledIndexesForInsertion(state.queue.shuffled, insertPosition, newUniqueIds.length);
                        // New items will be at indexes starting from insertPosition
                        const newIndexes = Array.from({ length: newUniqueIds.length }, (_, i) => insertPosition + i);
            ...
            case Play.NEXT_SHUFFLE: {
                set((state) => {
                    ...
                    if (isShuffleEnabled(state)) {
                        // Adjust existing indexes that are >= insertPosition
                        const adjustedShuffled = adjustShuffledIndexesForInsertion(state.queue.shuffled, insertPosition, shuffledIds.length);
                        // New items will be at indexes starting from insertPosition
            ...
            case Play.NOW: {
                set((state) => {
                    ...
                    if (state.player.shuffle === PlayerShuffle.TRACK) {
                        ...
                        if (targetSongUniqueId) {
                            const initialIndex = newUniqueIds.findIndex((id) => id === targetSongUniqueId);
                            if (initialIndex !== -1) {
                                const allIndexes = Array.from({ length: newUniqueIds.length }, (_, i) => i);
                                ...
                                state.queue.shuffled = generateShuffledIndexes(newUniqueIds.length);
                            }
                        }
                        else {
                            state.queue.shuffled = generateShuffledIndexes(newUniqueIds.length);
        ...
        set((state) => {
            ...
            if (state.player.shuffle === PlayerShuffle.TRACK) {
                ...
                if (currentTrackUniqueId) {
                    ...
                    const newIndexes = Array.from({ length: newUniqueIds.length }, (_, i) => insertIndex + i);
                    const currentShuffledIndex = state.player.index;
                    state.queue.shuffled = addIndexesToShuffled(adjustedShuffled, currentShuffledIndex, newIndexes);
                    // Recalculate player index to the shuffled position
                    const queueIndex = newQueue.findIndex((id) => id === currentTrackUniqueId);
                    if (queueIndex !== -1) {
                        const shuffledPosition = state.queue.shuffled.findIndex((idx) => idx === queueIndex);
                        if (shuffledPosition !== -1) {
                            state.player.index = shuffledPosition;
    ...
    clearSelected: (items) => {
        set((state) => {
            ...
            state.queue.default = state.queue.default.filter((id) => !uniqueIds.has(id));
            if (isShuffleEnabled(state)) {
    ...
    mediaAutoNext: () => {
        ...
        const { nextIndex: nextPlaybackIndex, shouldPause } = calculateNextIndex(currentIndex, playbackLength, repeat);
        const pauseOnNext = player.pauseOnNextSongEnd;
        const newStatus = shouldPause || pauseOnNext ? PlayerStatus.PAUSED : PlayerStatus.PLAYING;
        set((state) => {
        ...
        if (isShuffle) {
            currentQueueIndex = mapShuffledToQueueIndex(nextPlaybackIndex, stateSnapshot.queue.shuffled);
        }
    ...
    mediaPlay: (id) => {
        ...
        set((state) => {
            if (id) {
                ...
                const queueIndex = queue.items.findIndex((item) => item._uniqueId === id);
                if (queueIndex !== -1) {
                    if (state.player.shuffle === PlayerShuffle.TRACK &&
                        state.queue.shuffled.length > 0) {
                        // Find the shuffled position for this queue index
    ...
    mediaPlayByIndex: (index) => {
        ...
        set((state) => {
            ...
            if (isShuffleEnabled(state)) {
                // Find the shuffled position for this queue index
                const shuffledPosition = findShuffledPositionForQueueIndex(index, state.queue.shuffled);
    ...
    mediaSkipBackward: (offset) => {
        const offsetFromSettings = useSettingsStore.getState().general.skipButtons.skipBackwardSeconds;
    ...
    mediaSkipForward: (offset) => {
        const state = get();
        const queue = getQueueFromState(state);
        const index = state.player.index;
        const currentTrack = queue.items[index];
        const duration = currentTrack?.duration;
        const offsetFromSettings = useSettingsStore.getState().general.skipButtons.skipForwardSeconds;
        const timeToSkip = offset ?? offsetFromSettings ?? 5;
        if (!duration) {
            return;
        }
    ...
    moveSelectedTo: (items, uniqueId, edge) => {
    ...
    moveSelectedToBottom: (items) => {
        set((state) => {
            ...
            const filtered = state.queue.default.filter((id) => !uniqueIds.includes(id));
            const newQueue = [...filtered, ...uniqueIds];
            recalculatePlayerIndex(state, newQueue);
            state.queue.default = newQueue;
        });
    },
    moveSelectedToNext: (items) => {
        set((state) => {
            const uniqueIds = items.map((item) => item._uniqueId);
            // Add new songs to songs object
            items.forEach((item) => {
        ...
        set((state) => {
            ...
            if (willBeShuffled) {
                ...
                if (currentIndex >= 0 && currentIndex < state.queue.default.length) {
                    ...
                    const shuffledPosition = findShuffledPositionForQueueIndex(currentIndex, state.queue.shuffled);
    ...
    shuffleAll: () => {
        set((state) => {
            ...
            if (currentSong && currentIndex >= 0 && currentIndex < queue.items.length) {
                ...
                const currentQueueIndex = state.queue.default.findIndex((id) => id === currentUniqueId);
    ...
    shuffleSelected: (items) => {
        set((state) => {
            ...
            const selectedItems = selectedPositions.map((pos) => state.queue.default[pos]);
        ...
        set((state) => {
            ...
            if (willBeShuffled) {
                ...
                if (combinedLength > 0 &&
                    ...
                    const remainingIndexes = Array.from({ length: combinedLength }, (_, i) => i).filter((idx) => idx !== currentQueuePosition);
    ...
    partialize: (state) => {
        ...
        const persistQueue = resumeEnabled && isStructuredMusicContext(state.player.context);
        ...
        const player = Object.fromEntries(Object.entries(state.player).filter(([key]) => !excludedPlayerKeys.includes(key)));
        if (!persistQueue) {
            return { player };
        }
...
export const getCurrentSong = (state = usePlayerStoreBase.getState()) => getCurrentSongFromState(state);
export const getPlayerData = (state = usePlayerStoreBase.getState()) => getPlayerDataFromState(state);
export const getQueue = (groupBy, state = usePlayerStoreBase.getState()) => getQueueFromState(state, groupBy);
export const getQueueOrder = (state = usePlayerStoreBase.getState()) => getQueueOrderFromState(state);
export const isFirstTrackInQueue = (state = usePlayerStoreBase.getState()) => isFirstTrackInQueueFromState(state);
export const isLastTrackInQueue = (state = usePlayerStoreBase.getState()) => isLastTrackInQueueFromState(state);
export const usePlayerActions = () => {
    const actions = usePlayerStoreBase(useShallow((state) => ({
    ...
    return useMemo(() => ({
        ...actions,
        getQueue: (groupBy) => getQueueFromState(usePlayerStoreBase.getState(), groupBy),
        isFirstTrackInQueue: () => isFirstTrackInQueueFromState(usePlayerStoreBase.getState()),
        isLastTrackInQueue: () => isLastTrackInQueueFromState(usePlayerStoreBase.getState()),
        setTimestamp: setTimestampStore,
    }), [actions]);
};
export async function addToQueueByData(type, data) {
    claimMusicPlayback();
...
export const subscribePlayerQueue = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => state.queue, (queue, prevQueue) => {
        onChange(queue, prevQueue);
    });
};
export const subscribeCurrentTrack = (onChange) => {
...
export const subscribeNextSongInsertion = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => {
        const queue = getQueueFromState(state);
        ...
        const sameTrackStillPlaying = current.currentUniqueId !== undefined &&
...
export const subscribePlayerVolume = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => state.player.volume, (volume, prevVolume) => {
        onChange({ volume }, { volume: prevVolume });
    });
};
export const subscribePlayerStatus = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => state.player.status, (status, prevStatus) => {
...
export const subscribePlayerMute = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => state.player.muted, (muted, prevMuted) => {
        onChange({ muted }, { muted: prevMuted });
    });
};
export const subscribePlayerSpeed = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => state.player.speed, (speed, prevSpeed) => {
        onChange({ speed }, { speed: prevSpeed });
    });
};
export const subscribePlayerRepeat = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => state.player.repeat, (repeat, prevRepeat) => {
        onChange({ repeat }, { repeat: prevRepeat });
    });
};
export const subscribePlayerShuffle = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => state.player.shuffle, (shuffle, prevShuffle) => {
        onChange({ shuffle }, { shuffle: prevShuffle });
    });
};
export const subscribeQueueCleared = (onChange) => {
    return usePlayerStoreBase.subscribe((state) => state.queue, (queue, prevQueue) => {
...
export const usePlayerProperties = () => {
    return usePlayerStoreBase(useShallow((state) => ({
...
export const usePlayerVolumeState = () => usePlayerStoreBase(useShallow((state) => ({
    muted: state.player.muted,
    volume: state.player.volume,
})));
export const usePlayerPlaybackControlsState = () => usePlayerStoreBase(useShallow((state) => ({
    repeat: state.player.repeat,
    shuffle: state.player.shuffle,
    status: state.player.status,
})));
export const usePlayerMpvEngineState = () => usePlayerStoreBase(useShallow((state) => ({
    muted: state.player.muted,
    speed: state.player.speed,
    volume: state.player.volume,
})));
...
export const updateQueueFavorites = (ids, favorite) => {
    usePlayerStoreBase.setState((state) => {
...
export const updateQueueRatings = (ids, rating) => {
    usePlayerStoreBase.setState((state) => {
        Object.values(state.queue.songs).forEach((song) => {
            if (ids.includes(song.id)) {
...
export const incrementQueuePlayCount = (ids) => {
    usePlayerStoreBase.setState((state) => {
        Object.values(state.queue.songs).forEach((song) => {
            if (ids.includes(song.id)) {
...
export const updateQueueSong = (songId, updatedSong) => {
    usePlayerStoreBase.setState((state) => {
        Object.values(state.queue.songs).forEach((song) => {
            if (song.id === songId) {
...
export const usePlayerSong = () => {
    return usePlayerStoreBase((state) => {
        ...
        return (prev?._uniqueId === next?._uniqueId &&
            prev?.userFavorite === next?.userFavorite &&
            prev?.userRating === next?.userRating);
    });
};
export const usePlayerSongProperties = (properties) => {
    return usePlayerStoreBase(useShallow((state) => {
...
function cleanupOrphanedSongs(state) {
...
function recalculatePlayerIndex(state, queue) {
    const currentTrack = getCurrentSongFromState(state);
    if (!currentTrack) {
        return;
    }
...
function toQueueSong(item) {
    return {
        ...item,
        _uniqueId: nanoid(),
    };
}
export const usePlayerTransportSlice = () => usePlayerStoreBase(useShallow((state) => ({
    crossfadeDuration: state.player.crossfadeDuration,
    crossfadeStyle: state.player.crossfadeStyle,
...
export const usePlayerQueueSlice = () => usePlayerStoreBase(useShallow((state) => ({
