...
import { ArtistItem, HomeItem, } from './schemas';
export const createSettingsMigrate = (initialState) => (persistedState, version) => {
    const state = persistedState;
    if (version === 8) {
        state.general.sidebarItems = state.general.sidebarItems.filter((item) => item.id !== 'Folders');
    ...
    if (version <= 17) {
        ...
        if (state.lyrics &&
            ...
            const { fontSize, fontSizeUnsync, gap, gapUnsync, ...mainSettings } = defaultSettings;
    ...
    if (version <= 19) {
        ...
        if (state.visualizer?.audiomotionanalyzer?.presets &&
    ...
    if (version <= 20) {
        ...
        listKeysToUpdate.forEach((listKey) => {
            ...
            if (listConfig?.table?.columns) {
                ...
                const hasTitleArtist = columns.some((col) => col.id === TableColumn.TITLE_ARTIST);
                if (!hasTitleArtist) {
                    const titleCombinedIndex = columns.findIndex((col) => col.id === TableColumn.TITLE_COMBINED);
    ...
    if (version <= 21) {
        ...
        listKeysToUpdate.forEach((listKey) => {
            ...
            if (listConfig?.table?.columns) {
                ...
                if (!hasComposer) {
                    const artistIndex = columns.findIndex((col) => col.id === TableColumn.ARTIST);
                    if (artistIndex >= 0) {
    ...
    if (version <= 22) {
        ...
        Object.keys(state.lists).forEach((listKey) => {
            ...
            if (listConfig?.table &&
                ...
                listConfig.table.enableHeader = true;
            }
        });
    }
    if (version <= 23) {
        // Add FAVORITE_SONGS to album artist page configuration
        const hasFavoriteSongs = state.general.artistItems?.some((item) => item.id === ArtistItem.FAVORITE_SONGS);
        if (!hasFavoriteSongs) {
    ...
    if (version <= 29) {
        ...
        if (Array.isArray(state.lyrics?.sources)) {
            state.lyrics.sources = state.lyrics.sources.filter((s) => !removed.includes(s));
            if (state.lyrics.sources.length === 0) {
                state.lyrics.sources = [LyricSource.LRCLIB];
            }
        }
        delete state.lyrics?.enableNeteaseTranslation;
    }
    if (version <= 30) {
