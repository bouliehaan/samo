...
export const formatDateAbsolute = (key) => (key ? dayjs(key).format('ll') : '');
export const formatDateAbsoluteUTC = (key) => key ? dayjs.utc(key).format('ll') : '';
const PARTIAL_ISO_YEAR = /^\d{4}$/;
const PARTIAL_ISO_YEAR_MONTH = /^\d{4}-\d{2}$/;
export const formatPartialIsoDateUTC = (key) => {
    if (!key) {
        return '';
    }
    ...
    if (PARTIAL_ISO_YEAR.test(trimmedKey)) {
        ...
        return new Intl.DateTimeFormat(intlLocale, { timeZone: 'UTC', year: 'numeric' }).format(new Date(Date.UTC(year, 0, 1)));
    }
...
export const formatHrDateTime = (key) => (key ? dayjs(key).format('lll') : '');
export const formatDateRelative = (key) => (key ? dayjs(key).fromNow() : '');
...
export const formatDurationStringShort = (duration) => {
...
export const formatSizeString = (size) => {
    let count = 0;
