...
import { ExplicitStatus, LibraryItem, ServerType, } from '/@/shared/types/domain-types';
const getArtistList = (artists, artistId, artistName, participants) => {
...
const normalizeSong = (item, server, pathReplace, pathReplaceWith, playlistIndex, discTitleMap) => {
    const participants = getParticipants(item);
    const albumArtistsList = getArtistList(item.albumArtists, item.artistId, item.artist);
    const albumArtistName = item.albumArtists?.length > 0
    ...
    return {
        ...
        explicitStatus: item.explicitStatus === 'explicit'
            ? ExplicitStatus.EXPLICIT
            : item.explicitStatus === 'clean'
                ? ExplicitStatus.CLEAN
                : null,
        gain: item.replayGain && (item.replayGain.albumGain || item.replayGain.trackGain)
            ? {
        ...
        peak: item.replayGain && (item.replayGain.albumPeak || item.replayGain.trackPeak)
            ? {
...
const normalizeAlbum = (item, server, pathReplace, pathReplaceWith) => {
    const discTitleMap = new Map();
    item.discTitles?.forEach((discTitle) => {
    ...
    return {
        ...
        songs: item.song?.map((song) => normalizeSong(song, server, pathReplace, pathReplaceWith, undefined, discTitleMap)) || [],
        sortName: item.title,
        tags: null,
...
const normalizeFolder = (item, server, pathReplace, pathReplaceWith) => {
    const results = item.child?.reduce((acc, item) => {
...
const normalizeInternetRadioStation = (item) => {
    return {
