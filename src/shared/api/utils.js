...
import { z } from 'zod';
import { AlbumArtistListSort, AlbumListSort, LibraryItem, RadioListSort, SongListSort, SortOrder, } from '/@/shared/types/domain-types';
// Since ts-rest client returns a strict response type, we need to add the headers to the body object
export const resultWithHeaders = (itemSchema) => {
    return z.object({
        data: itemSchema,
        headers: z.instanceof(AxiosHeaders),
    });
};
export const resultSubsonicBaseResponse = (itemSchema) => {
    return z.object({
        'subsonic-response': z
            .object({
            status: z.string(),
...
export const hasFeature = (server, feature) => {
    if (!server || !server.features) {
        return false;
    }
    return (server.features[feature]?.length || 0) > 0;
};
export const hasFeatureWithVersion = (server, feature, version) => {
    if (!server || !server.features) {
        return false;
    }
 ...
 */
export const getFeatures = (versionInfo, version) => {
...
export const sortSongList = (songs, sortBy, sortOrder) => {
    let results = songs;
    const order = sortOrder === SortOrder.ASC ? 'asc' : 'desc';
    switch (sortBy) {
        case SongListSort.ALBUM:
            results = orderBy(results, [(v) => v.album?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order]);
            break;
        ...
        case SongListSort.ARTIST:
            results = orderBy(results, [(v) => v.artistName?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order, order]);
            break;
        case SongListSort.BPM:
            results = orderBy(results, ['bpm', (v) => v.album?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order, order]);
            break;
        case SongListSort.CHANNELS:
            results = orderBy(results, ['channels', (v) => v.album?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order, order]);
            break;
        case SongListSort.COMMENT:
            results = orderBy(results, ['comment', (v) => v.album?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order, order]);
            break;
        case SongListSort.DURATION:
            results = orderBy(results, ['duration', (v) => v.album?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order, order]);
            break;
        ...
        case SongListSort.NAME:
            results = orderBy(results, [(v) => v.name.toLowerCase(), (v) => v.album?.toLowerCase()], [order, order]);
            break;
        case SongListSort.PLAY_COUNT:
            results = orderBy(results, ['playCount', (v) => v.album?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order, order]);
            break;
        ...
        case SongListSort.RECENTLY_PLAYED:
            results = orderBy(results, ['lastPlayedAt', (v) => v.album?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order, order]);
            break;
        case SongListSort.RELEASE_DATE:
            results = orderBy(results, ['releaseDate', (v) => v.album?.toLowerCase(), 'discNumber', 'trackNumber'], [order, order, order, order]);
            break;
        case SongListSort.SORT_NAME:
            results = orderBy(results, [(v) => v.sortName ?? v.name], [order]);
            break;
        case SongListSort.YEAR:
            results = orderBy(results, ['releaseYear', (v) => v.album?.toLowerCase(), 'discNumber', 'track'], [order, order, order, order]);
            break;
        default:
            break;
    }
...
export const sortSongsByFetchedOrder = (songs, fetchedIds, itemType) => {
    ...
    const songsByFetchedId = new Map();
    for (const song of songs) {
        ...
        switch (itemType) {
            ...
            case LibraryItem.ALBUM_ARTIST:
                matchedId = fetchedIds.find((id) => song.albumArtists.some((artist) => artist.id === id));
                break;
            case LibraryItem.ARTIST:
                matchedId = fetchedIds.find((id) => song.artists.some((artist) => artist.id === id));
                break;
    ...
    if (unmatchedSongs.length > 0) {
        ...
        if (itemType === LibraryItem.ALBUM_ARTIST) {
            ...
            const sortedUnmatched = orderBy(unmatchedSongs, ['discNumber', 'trackNumber'], ['asc', 'asc']);
            result.push(...sortedUnmatched);
        }
    }
    return result;
};
export const sortAlbumArtistList = (artists, sortBy, sortOrder) => {
    const order = sortOrder === SortOrder.ASC ? 'asc' : 'desc';
    let results = artists;
    switch (sortBy) {
...
export const sortAlbumList = (albums, sortBy, sortOrder) => {
    let results = albums;
    const order = sortOrder === SortOrder.ASC ? 'asc' : 'desc';
    switch (sortBy) {
        case AlbumListSort.ALBUM_ARTIST:
            results = orderBy(results, ['albumArtist', (v) => v.name.toLowerCase()], [order, 'asc']);
            break;
...
export const sortRadioList = (stations, sortBy, sortOrder) => {
...
export const replacePathPrefix = (path, replacePrefix, addPrefix) => {
    let newPath = path;
