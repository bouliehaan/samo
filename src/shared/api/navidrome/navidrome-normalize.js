...
//     return url;
// };
const navidromeImageIdWithCacheBust = (id, uploadedImage, updatedAt) => (!uploadedImage ? id : `${id}&_=${updatedAt ?? ''}`);
const normalizePlayDate = (item) => {
...
const getArtists = (item, includeRemixers = true) => {
    let albumArtists;
    let artists;
    let remixers;
...
const normalizeSong = (item, server, pathReplace, pathReplaceWith) => {
    ...
    const releaseYear = fromSongRelease.year > 0 ? fromSongRelease.year : songApiYear > 0 ? songApiYear : null;
    ...
    return {
        ...
        explicitStatus: item.explicitStatus === 'e'
            ? ExplicitStatus.EXPLICIT
            : item.explicitStatus === 'c'
                ? ExplicitStatus.CLEAN
                : null,
        gain: item.rgAlbumGain || item.rgTrackGain
        ...
        peak: item.rgAlbumPeak || item.rgTrackPeak
...
const normalizeAlbumArtist = (item, server) => {
    ...
    let songCount;
    if (item.stats) {
        albumCount = Math.max(item.stats.albumartist?.albumCount ?? 0, item.stats.artist?.albumCount ?? 0);
        songCount = Math.max(item.stats.albumartist?.songCount ?? 0, item.stats.artist?.songCount ?? 0);
    }
    else {
        albumCount = item.albumCount;
        songCount = item.songCount;
    }
    const imageId = navidromeImageIdWithCacheBust(item.id, item.uploadedImage, item.updatedAt ?? item.externalInfoUpdatedAt);
    return {
