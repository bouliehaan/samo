...
export function BaseSkeleton({ baseColor, borderRadius, className, containerClassName, count = 1, direction = 'ltr', enableAnimation = true, height, inline, isCentered, style, width, }) {
    ...
    return (<div className={clsx(containerClasses, styles.skeletonWrapper)} dir={direction} ref={containerRef}>
            {Array.from({ length: count }, (_, i) => (<div className={skeletonClasses} key={i} style={skeletonStyle}/>))}
        </div>);
}
