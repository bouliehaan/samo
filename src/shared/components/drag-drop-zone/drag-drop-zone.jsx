...
const DragDropZoneText = ({ icon, onItemSelected, validateItem }) => {
    ...
    return (<Flex align="center" bd={`2px dashed ${borderColour}`} bdrs={'sm'} direction="column" gap={'sm'} justify="center" onClick={onZoneClick} onDragOver={onDragOver} onDrop={onItemDropped} p="sm" style={{ cursor: 'pointer' }}>
            <Icon icon={icon} size="3xl"/>
            <Text>{t('dragDropZone.mainText').toString()}</Text>
            {hasErrored ? (<Text c="red" ta="center">
                    {error}
                </Text>) : null}
            <input onChange={onZoneInputChange} ref={(self) => {
...
const DragDropZoneFile = (props) => {
    ...
    return (<div {...divProps} className={clsx(className, {
