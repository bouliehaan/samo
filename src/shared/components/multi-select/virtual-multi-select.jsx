...
export function VirtualMultiSelect({ disabled = false, displayCountType = 'album', height, isLoading = false, label, onChange, options, RowComponent, singleSelect = false, value, }) {
    ...
    return (<div className={clsx(styles.container, {
            ...
            <TextInput disabled={disabled} label={labelWithClear} leftSection={value.length > 0 && !disabled ? (<ActionIcon icon="x" iconProps={{ size: 'md' }} onClick={() => {
                onChange(null);
                setSearch('');
        ...
        }} placeholder={placeholder} rightSection={<Group gap="xs" wrap="nowrap">
                        {search && !disabled ? (<ActionIcon icon="x" iconProps={{ size: 'md' }} onClick={() => setSearch('')} size="xs" variant="subtle"/>) : (<Icon icon="search"/>)}
                    </Group>} styles={{
            ...
            <div className={styles.listContainer} onKeyDown={handleKeyDown} onMouseDown={(e) => {
            if (disabled)
                return;
        ...
        }} ref={listContainerRef} style={{ height: `${height}px` }} tabIndex={disabled ? -1 : 0}>
                {isLoading ? (<Center h="100%" w="100%">
                        <Spinner container/>
                    </Center>) : stableOptions.length === 0 ? (<Center h="100%" w="100%">
                        <Text isMuted isNoSelect size="sm">
                            {t('common.noResultsFromQuery', { postProcess: 'sentenceCase' })}
                        </Text>
                    </Center>) : (<List listRef={listRef} rowComponent={RowComponent} rowCount={stableOptions.length} rowHeight={rowHeight} rowProps={{
                disabled,
                displayCountType,
                focusedIndex,
            ...
            {selectedOptions.length > 0 && (<Stack gap="xs" mt="sm">
                    {selectedOptions.map((option) => (<Group className={clsx(styles.selectedOption, {
                    [styles.disabled]: disabled,
                })} gap="sm" key={option.value} onClick={() => handleDeselect(option.value)} wrap="nowrap">
                            {!disabled && (<ActionIcon icon="minus" iconProps={{ size: 'sm' }} onClick={(e) => {
                        e.stopPropagation();
                        handleDeselect(option.value);
                    }} size="xs" stopsPropagation variant="transparent"/>)}
