...
export const CheckboxSelect = ({ data, enableDrag, onChange, value }) => {
    ...
    return (<div className={styles.container}>
            {data.map((option) => (<CheckboxSelectItem enableDrag={enableDrag} key={option.value} onChange={handleChange} option={option} values={value}/>))}
        </div>);
};
function CheckboxSelectItem({ enableDrag, onChange, option, values }) {
    ...
    return (<div className={clsx(styles.item, {
            ...
            {enableDrag && (<ActionIcon className={styles.dragHandle} icon="dragVertical" ref={dragHandleRef} size="xs" variant="default"/>)}
            <Checkbox checked={values.includes(option.value)} label={option.label} onChange={(e) => {
