...
import { getSubsonicMusicQuality } from './mobile-playback';
...
export const annotateSubsonicAlbumsQuality = async (authentication, fetcher, items, limit = 80) => {
    ...
    for (let index = 0; index < scannedItems.length; index += QUALITY_SCAN_CONCURRENCY) {
        ...
        await Promise.all(chunk.map(async (item) => {
            try {
                const profile = await loadSubsonicAlbumQualityProfile(authentication, fetcher, item.id);
                if (profile) {
 ...
 * alias can go away.
 */
export const annotateSubsonicHiResCollections = async (authentication, fetcher, kind, items, limit = 80) => {
