...
import { appendAudiobookshelfAuthToken, buildRadioPlayback, buildSubsonicMusicPlayback, CHROMECAST_MAX_LOSSLESS_SAMPLE_RATE_HZ, getSubsonicMusicQuality, isSubsonicSongHiRes, mimeFromAudiobookshelfExt, needsChromecastCompatibleStream, } from './mobile-playback';
...
describe('appendAudiobookshelfAuthToken', () => {
    it('appends token with ? when the URL has no query', () => {
        expect(appendAudiobookshelfAuthToken('https://abs.example.com/play', 'abc+def')).toBe('https://abs.example.com/play?token=abc%2Bdef');
    });
    it('appends token with & when the URL already has query params', () => {
        expect(appendAudiobookshelfAuthToken('https://abs.example.com/play?foo=1', 'token')).toBe('https://abs.example.com/play?foo=1&token=token');
    });
});
...
describe('needsChromecastCompatibleStream', () => {
    it('flags sample rates above the Cast FLAC ceiling', () => {
        expect(needsChromecastCompatibleStream({
            container: 'flac',
            deliveryKind: 'android-direct',
            losslessRequired: true,
