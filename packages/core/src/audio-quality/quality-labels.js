...
const normalizeContainerKey = (container) => {
    if (!container)
        return '';
    ...
    if (withoutVendorPrefix === 'mpeg')
        return 'mp3';
    if (withoutVendorPrefix === 'wave')
        return 'wav';
    return withoutVendorPrefix;
};
const normalizeBitRateKbps = (bitRate) => {
    if (!bitRate)
        return null;
    return bitRate >= 100_000 ? bitRate / 1000 : bitRate;
};
export const formatContainer = (container) => {
    if (!container)
        return null;
    return container.toUpperCase();
};
export const formatBitDepth = (bitDepth, compact) => {
    if (!bitDepth)
        return null;
    return compact ? `${bitDepth}` : `${bitDepth}-bit`;
};
export const formatSampleRate = (sampleRate, compact) => {
    if (!sampleRate)
        return null;
...
export const isPremiumQualityContainer = (container) => {
    return PREMIUM_QUALITY_CONTAINERS.has(normalizeContainerKey(container));
};
export const isHiResAudioQuality = ({ bitDepth, bitRate, container, deliveryKind, sampleRate, }) => {
 ...
 * elsewhere.
 */
export const isLosslessAudioQuality = ({ bitDepth, bitRate, container, deliveryKind, sampleRate: _sampleRate, }) => {
    if (deliveryKind === 'transcoded') {
        return false;
    }
    if (!isPremiumQualityContainer(container)) {
...
export const getQualityLabel = ({ bitDepth, bitRate, container, isTranscoded, sampleRate, }) => {
...
const getPathLabel = (deliveryKind, compact) => {
    if (deliveryKind === 'transcoded') {
        return compact ? 'Transcoded' : 'Transcoded Compatibility';
    }
    if (deliveryKind === 'native-direct')
        return 'Native Direct';
    if (deliveryKind === 'android-direct')
        return 'DIRECT';
    if (deliveryKind === 'web-direct')
        return 'Web Direct';
    return 'Unknown Path';
};
export const buildAudioQualityBadgeItems = ({ bitDepth: sourceBitDepth, bitRate: sourceBitRate, compact = false, container: sourceContainer, deliveryKind, inline = false, mode = 'detail', sampleRate: sourceSampleRate, transcodeBitrate, transcodeFormat, }) => {
    ...
    const pathTone = isTranscoded
        ? 'transcoded'
        : isPremiumQualityDirect
            ? 'direct'
            : 'neutral';
    const detailTone = isPremiumQualityDirect ? 'direct' : 'neutral';
    ...
    if (mode === 'playerbar') {
        ...
        const qualityTone = isTranscoded && qualityLabel
            ? 'transcoded'
            : !qualityLabel
                ? 'unknown'
                : isPremiumQualityDirect
    ...
    if (isPremiumQualityDirect) {
        if (compact || inline) {
            items.push(bitDepth && sampleRate
            ...
            items.push(bitDepth
                ...
                : { label: 'Unknown bit depth', tone: 'unknown' }, sampleRate
